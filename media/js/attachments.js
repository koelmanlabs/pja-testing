...
jQuery(document).ready(function ($) {
    ...
    $('.attach-remove').on('click', function (event) {
        ...
        if (pos >= 0) {
            ...
            url = 'index.php?option=com_planjeagenda&task=ajaxattachremove&format=raw&id=' + id + '&' + token + '=1';
        } else {
            // var id = clickednode.id.substr(13);
            var id = $(this).attr('id').substr(13);
            url = 'index.php?option=com_planjeagenda&task=ajaxattachremove&format=raw&id=' + id;
        }
