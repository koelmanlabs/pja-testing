 ...
 * @package JEM
 */
abstract class PlanjeagendaUserAbstract extends User
{
    /**
     * @var    array  PlanjeagendaUser instances container.
     */
    ...
    static protected function _getInstance($id = 0)
    {
        ...
        if (empty(self::$instances_jemuser[$id]) || !(self::$instances_jemuser[$id] instanceof PlanjeagendaUser))
        {
            $user = new PlanjeagendaUser($id);
    ...
    public function editaccess($allowowner, $ownerid, $recurse, $level)
    {
        ...
        if ((($allowowner == 1) || $this->authorise('core.edit.own','com_planjeagenda')) && ($this->id == $ownerid) && ($ownerid != 0)) {
            return true;
     ...
     *
     * @deprecated Use PlanjeagendaUser::can($action, 'event', $eventid) instead.
     */
    public function ismaintainer($action, $eventid = false)
    {
        ...
        $query = 'SELECT gr.id' . ' FROM #__pja_groups AS gr'
        ...
        if ($action == 'edit') {
            ...
            $query = 'SELECT catid FROM #__pja_events WHERE id = ' . (int)$eventid;
            $db->setQuery($query);
        }
        else {
            $query = 'SELECT id' . ' FROM #__pja_categories'
                . ' WHERE published = 1'
     ...
     *                      must be valid field names of #__pja_groups table. (optional)
    ...
    public function getJemGroups($asset = false)
    {
        ...
        $query = 'SELECT gr.*'
            . ' FROM #__pja_groups AS gr'
            . ' LEFT JOIN #__pja_groupmembers AS gm ON gm.group_id = gr.id'
     ...
     */
    public function getJemCategories($actions = [], $type = 'event'): array
    {
        return []; // Replaced by standard Joomla ACL
    }
    ...
    public function can($action, $type, $id = false, $created_by = false, $categoryIds = false)
    {
        ...
        // not supported yet
        //foreach($categoryIds as $catId) {
        //    $assets[] = 'com_planjeagenda.category.'.$catId;
        //}
            ...
            foreach ($action as $act) {
                ...
                switch ($act) {
                    case 'add':
                        ...
                        break;
                    case 'edit':
                        $authorised |= $this->authorise('core.edit', $asset); // $edit is limited to events not attached to klevents groups
        ...
        if (!$authorised) {
            if (($type == 'event') || ($type == 'venue')) {
                ...
                if (!empty($jemgroups)) {
                    if (empty($categoryIds) && (($type != 'event') || (empty($id) && (!in_array('publish', $action))))) {
                    ...
                    } else { // we have a valid event object so check event's categories against klevents groups
                        ...
                        if (!empty($id)) {
                            $query .= ' LEFT JOIN #__pja_events AS ae ON ae.catid = c.id'
                                . ' WHERE ae.id = ' . (int)$id
                                . ' AND c.published = 1';
                        } else {
                        ...
                        foreach($cats as $cat) {
                            if (empty($cat->groupid)) {
                                ...
                                if (in_array($cat->access, $levels) && array_key_exists($cat->groupid, $jemgroups)) {
                                    // user can "see" this category and is member of connected klevents group granting permission
                                    $authorised = true;
                                    break; // foreach cats
                                }
                            }
 ...
 * @package JEM
 *
 * @see PlanjeagendaUserAbstract
 */
class PlanjeagendaUser extends PlanjeagendaUserAbstract
{
