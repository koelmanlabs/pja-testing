 ...
 * @package JEM
 */
class PlanjeagendaAttachment extends CMSObject
{
    /**
    ...
    {
        PlanjeagendaDebug::log('attachment upload', 'object=' . $object . ' files=' . count((array)$post_files));
        ...
        foreach ($post_files as $k => $rec)
        {
            ...
            if (!in_array($fileext, $allowed)) {
                Factory::getApplication()->enqueueMessage(Text::_('com_planjeagenda_ERROR_ATTACHEMENT_EXTENSION_NOT_ALLOWED').': '.$file, 'warning');
                continue;
            }
            // check size
            if ($rec['size'] > $maxsizeinput) {
                Factory::getApplication()->enqueueMessage(Text::sprintf('com_planjeagenda_ERROR_ATTACHEMENT_FILE_TOO_BIG', $file, $rec['size'], $maxsizeinput), 'warning');
                continue;
            }
            ...
            if (!Folder::exists($path)) {
                ...
                if (!$res) {
                    Factory::getApplication()->enqueueMessage(Text::_('com_planjeagenda_ERROR_COULD_NOT_CREATE_FOLDER').': '.$path, 'warning');
                    return false;
                }
            }
            ...
            $sanitizedFilename = PlanjeagendaImage::sanitize($path, $file);
            ...
            $table = (new \KoelmanLabs\Component\Planjeagenda\Administrator\Table\jem_attachments(\Joomla\CMS\Factory::getContainer()->get('DatabaseDriver')));
            ...
            if (!($table->check() && $table->store())) {
                \Joomla\CMS\Factory::getApplication()->enqueueMessage(Text::_('com_planjeagenda_ERROR_ATTACHMENT_SAVING_TO_DB').': '.$table->getError(), 'warning');
            }
        } // foreach
    ...
    static public function update($attach)
    {
        ...
        if (!($table->check() && $table->store())) {
            \Joomla\CMS\Factory::getApplication()->enqueueMessage(Text::_('com_planjeagenda_ERROR_ATTACHMENT_UPDATING_RECORD').': '.$table->getError(), 'warning');
            return false;
        }
    ...
    static public function getAttachmentPath($id)
    {
        ...
        if (!$res) {
            throw new Exception(Text::_('com_planjeagenda_FILE_NOT_FOUND'), 404);
        }

        if (!in_array($res->access, $levels)) {
            throw new Exception(Text::_('com_planjeagenda_NO_ACCESS'), 403);
        }

        $path = \Joomla\CMS\Filesystem\Path::clean(JPATH_SITE.'/'.$jemsettings->attachments_path.'/'.$res->object.'/'.$res->file);
        $basePath = \Joomla\CMS\Filesystem\Path::clean(JPATH_SITE.'/'.$jemsettings->attachments_path);
    ...
    {
        PlanjeagendaDebug::log('attachment remove', 'id=' . $id);
        $jemsettings = PlanjeagendaHelper::config();
        ...
        if (empty($userid) || ($userid != $res->added_by)) {
            ...
            if (!$user->authorise('core.edit', 'com_planjeagenda')) {
                PlanjeagendaHelper::addLogEntry("User {$userid} is not permritted to remove attachment " . $res->object, __METHOD__);
                return false;
            }
        }

        PlanjeagendaHelper::addLogEntry("User {$userid} removes attachment " . $res->object.'/'.$res->file, __METHOD__);
        $path = \Joomla\CMS\Filesystem\Path::clean(JPATH_SITE.'/'.$jemsettings->attachments_path.'/'.$res->object.'/'.$res->file);
        $basePath = \Joomla\CMS\Filesystem\Path::clean(JPATH_SITE.'/'.$jemsettings->attachments_path);
        if (strpos($path, $basePath) === 0 && file_exists($path)) {
            File::delete($path);
        ...
        $query = 'DELETE FROM #__pja_attachments '
