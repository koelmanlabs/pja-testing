...
// ensure PlanjeagendaFactory is loaded (because this class is used by modules or plugins too)
require_once(JPATH_SITE.'/components/com_planjeagenda/factory.php');
if (!class_exists('PlanjeagendaHtml')) {
    require_once JPATH_ADMINISTRATOR . '/components/com_planjeagenda/helpers/html/kleventshtml.php';
}
...
static public function lightbox() {
    ...
    static public function createButtonBar($view, $permissions, $params)
    {
        ...
        if (!$archive) {
            if (in_array('addEvent', $btns_show) || (!in_array('addEvent', $btns_hide) && in_array($view, array('calendar', 'categories', 'category', 'day', 'event', 'eventslist', 'myevents', 'myvenues', 'venue', 'venues')))) {
                $buttons[$idx][] = PlanjeagendaOutput::submitbutton(!empty($canAddEvent), null);
            }
            if (in_array('addVenue', $btns_show) || (!in_array('addVenue', $btns_hide) && in_array($view, array('calendar', 'categories', 'category', 'day', 'event', 'eventslist', 'myevents', 'myvenues', 'venue', 'venues', 'venueslist')))) {
                $buttons[$idx][] = PlanjeagendaOutput::addvenuebutton(!empty($canAddVenue), null, null);
            }
            if (in_array('addUsers', $btns_show) || (!in_array('addUsers', $btns_hide) && in_array($view, array('attendees')))) {
                $buttons[$idx][] = PlanjeagendaOutput::addusersbutton(!empty($canAddUsers), $id);
            }
        }
        ...
        if (in_array('archive', $btns_show) || (!in_array('archive', $btns_hide) && in_array($view, array('categories', 'category', 'eventslist', 'myattendances', 'myevents', 'venue')))) {
            $buttons[$idx][] = PlanjeagendaOutput::archivebutton($archive_link, $task , $id); // task: archive, id: for '&id='
        }
        if (in_array('mail', $btns_show) || (!in_array('mail', $btns_hide) && in_array($view, array('category', 'event', 'venue', 'venueslist')))) {
            $buttons[$idx][] = PlanjeagendaOutput::mailbutton($slug, $view, null); // slug: for '&id='
        }
    ...
    static public function addvenuebutton($addvenuelink, $params, $settings2)
    {
        if ($addvenuelink) {
            ...
            if ($settings->get('global_show_icons',1)) {
                $image = PlanjeagendaHtml::icon( 'com_planjeagenda/addvenue.webp', 'fa fa-fw fa-lg fa-plus-square klevents-addvenuebutton', Text::_('com_planjeagenda_DELIVER_NEW_VENUE'), NULL, !$app->isClient('site'));
            } else {
    ...
    static public function archivebutton($archive_link, $task = NULL, $id = NULL)
    {
        ...
        if ($settings->get('global_show_archive_icon',1)) {
            ...
            if ($task == 'archive') {
                if ($settings->get('global_show_icons',1)) {
                    $image = PlanjeagendaHtml::icon( 'com_planjeagenda/el.webp', 'fa fa-fw fa-lg fa-calendar klevents-archivebutton-return', Text::_('com_planjeagenda_SHOW_EVENTS'), NULL, !$app->isClient('site'));
                } else {
                ...
                if ($settings->get('global_show_icons',1)) {
                       $image = PlanjeagendaHtml::icon( 'com_planjeagenda/archive_front.webp', 'fa fa-fw fa-lg fa-archive klevents-archivebutton', Text::_('com_planjeagenda_SHOW_ARCHIVE'), NULL, !$app->isClient('site'));
                } else {
    ...
    static public function printbutton($print_link, $params)
    {
        ...
        if ($settings->get('global_show_print_icon',0)) {
            ...
            if ($settings->get('global_show_icons',1)) {
                $image = PlanjeagendaHtml::icon( 'com_planjeagenda/printButton.webp', 'fa fa-fw fa-lg fa-print klevents-printbutton', Text::_('JGLOBAL_PRINT'), NULL, !$app->isClient('site'));
            } else {
    ...
    static public function icalbutton($slug, $view, $task = null)
    {
        ...
        if ((int)$settings->get('global_show_ical_icon', 0) === 1) {
            ...
            if ((int)$settings->get('global_show_icons', 0) === 1) {
                $image = PlanjeagendaHtml::icon( 'com_planjeagenda/iCal2.0.webp', 'fa fa-fw fa-lg fa-calendar-check klevents-icalbutton', Text::_('com_planjeagenda_EXPORT_ICS'), NULL, !$app->isClient('site'));
            } else {
            ...
            function ($category) use ($doLink, $backend) {
                if ($doLink) {
                    if ($backend) {
                        ...
                        $value  = '<a href="' . Route::_(PlanjeagendaHelperRoute::getCategoryRoute($category->catslug)) . '">' .
                                      $category->catname . '</a>';
                    }
                } else {
...
endif; // class_exists PlanjeagendaOutput
