...
require_once(JPATH_SITE.'/components/com_planjeagenda/classes/Zebra_Image.php');
...
class PlanjeagendaImage
{
    ...
    static public function flyercreator($image, $type)
    {
        ...
        if ($image) {
            $img_orig  = 'images/klevents/'.$folder.'/'.$image;
            $img_thumb = 'images/klevents/'.$folder.'/small/'.$image;
            ...
            if ($settings->gddisabled == 1 && !file_exists($save)) {
                PlanjeagendaImage::thumb($filepath, $save, $settings->imagewidth, $settings->imagehight);
            }
    ...
    static public function check($file, $jemsettings)
    {
        ...
        if (!getimagesize($file['tmp_name'])) {
            Factory::getApplication()->enqueueMessage(Text::_('com_planjeagenda_UPLOAD_FAILED_NOT_AN_IMAGE').': '.htmlspecialchars($file['name'], ENT_COMPAT, 'UTF-8'), 'warning');
            return false;
        }
        ...
        if (!in_array($fileext, $allowable)) {
            Factory::getApplication()->enqueueMessage(Text::_('com_planjeagenda_WRONG_IMAGE_FILE_TYPE').': '.htmlspecialchars($file['name'], ENT_COMPAT, 'UTF-8'), 'warning');
            return false;
        }

        //Check filesize
        if ($imagesize > $sizelimit) {
            Factory::getApplication()->enqueueMessage(Text::_('com_planjeagenda_IMAGE_FILE_SIZE').': '.htmlspecialchars($file['name'], ENT_COMPAT, 'UTF-8'), 'warning');
            return false;
        }
        ...
        foreach ($html_tags as $tag) {
            ...
            if (stristr($xss_check, '<'.$tag.' ') || stristr($xss_check, '<'.$tag.'>')) {
                Factory::getApplication()->enqueueMessage(Text::_('com_planjeagenda_WARN_IE_XSS'), 'warning');
                return false;
            }
        }
