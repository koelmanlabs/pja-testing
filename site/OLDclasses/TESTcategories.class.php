...
class PlanjeagendaCategories
{
    ...
    {
        self::$instances = new PlanjeagendaCategories($cid, $options);
    ...
    */
    ...
    protected function _load($id)
    {
        ...
        $query->join('LEFT', '#__pja_events AS ae ON ae.catid = c.id');
        ...
        */
        ...
        if ($id != 'root')
        {
            ...
            {
                $query->leftJoin('#__pja_categories AS s ON (s.lft <= c.lft AND s.rgt >= c.rgt) OR (s.lft > c.lft AND s.rgt < c.rgt)');
            }
        }

        $subQuery = ' (SELECT cat.id as id FROM #__pja_categories AS cat JOIN #__pja_categories AS parent'
        ...
        if (isset($this->_options['countItems']) && $this->_options['countItems'] == 1)
        {
            ...
            $query->leftJoin('#__pja_events AS i ON ae.id = i.id'.' AND ' . $where . ' AND i.access IN ('.implode(',', $levels).')');
        ...
        if (is_array($results) && count($results))
        {
            ...
            foreach ($results as $result)
            {
                ...
                if (!isset($this->_nodes[$result->id]))
                {
                    ...
                    $this->_nodes[$result->id] = new PlanjeagendaCategoryNode($result, $this);
    ...
    public function getCategories($id = 0)
    {
        ...
        $query->join('LEFT','#__pja_events AS a ON a.id = ae.id');

        if ($id != 'all'){
            $query->where('ae.id ='.(int)$id);
        }
    ...
    static protected function buildParentCats($cid)
    {
        ...
        // start with parent
        $query = 'SELECT parent_id FROM #__pja_categories WHERE id = ' . (int) $cid;
        $db->setQuery($query);
    ...
    static public function getCategoriesTree($published = false)
    {
        ...
        $query = 'SELECT *, id AS value, catname AS text' . ' FROM #__pja_categories' . $where . ' ORDER BY ' . $order;
    ...
    static public function buildcatselect($list, $name, $selected, $top, $class = array('class' => 'inputbox'))
    {
        ...
        if ($top) {
            $catlist[] = HTMLHelper::_('select.option', '0', Text::_('com_planjeagenda_TOPLEVEL'));
        }
    ...
    static public function getChilds($id)
    {
        ...
        $user = \Joomla\CMS\Factory::getApplication()->getIdentity();
        $levels = $user->getAuthorisedViewLevels();
        $query = ' SELECT id, parent_id ' . ' FROM #__pja_categories ' . ' WHERE published = 1 AND access IN ('.implode(',', $levels).')';
        $db->setQuery($query);
 ...
 */
class PlanjeagendaCategoryNode extends CMSObject
{
    ...
    */
    ...
    {
        if ($parent instanceof PlanjeagendaCategoryNode || is_null($parent))
        {
    ...
    {
        if ($child instanceof PlanjeagendaCategoryNode)
        {
...
endif; // class_exists PlanjeagendaCategories
