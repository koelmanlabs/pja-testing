...
if (!class_exists('PlanjeagendaDebug', false)) {
    require_once JPATH_SITE . '/components/com_planjeagenda/classes/debug.php';
}
...
class PlanjeagendaHelper
{
    ...
    {
        PlanjeagendaDebug::log('cleanup', 'forced=' . $forced);
        ...
        if (($nrdaysnow > $nrdaysupdate) || $forced) {
            PlanjeagendaHelper::addLogEntry('forced: ' . $forced . ', now: '. $now . ', last update: ' . $lastupdate .
            ...
            if (($runningupdate + $delay) < $now) {
                ...
                PlanjeagendaConfig::getInstance()->set('runningupdate', $now);

                PlanjeagendaHelper::addLogEntry('  do cleanup...', __METHOD__);

                // trigger an event to let plugins handle whatever cleanup they want to do.
                if (PluginHelper::importPlugin('planjeagenda')) {
                    Factory::getApplication()->triggerEvent('onJemBeforeCleanup', array($jemsettings, $forced));
                }
                ...
                foreach ($recurrence_array as $recurrence_row)
                {
                    ...
                    $ref_event = new \KoelmanLabs\Component\Planjeagenda\Administrator\Table\EventTable($db);
                    $ref_event->load($recurrence_row['id']);

                    $db = Factory::getContainer()->get('DatabaseDriver');
                    $query = $db->getQuery(true);
                    $query->select('*');
                    $query->from($db->quoteName('#__pja_events').' AS a');
                    ...
                    $recurrence_row = PlanjeagendaHelper::calculate_recurrence($recurrence_row);

                    switch ($recurrence_row["recurrence_type"]) {
                        case 1:
                            $anticipation    = min(36, (int)$jemsettings->recurrence_anticipation_day);
                            break;
                        case 2:
                            $anticipation    = min(36, (int)$jemsettings->recurrence_anticipation_week);
                            break;
                        case 3:
                            $anticipation    = min(36, (int)$jemsettings->recurrence_anticipation_month);
                            break;
                        case 4:
                            $anticipation    = min(36, (int)$jemsettings->recurrence_anticipation_week);
                            break;
                        case 5:
                            $anticipation    = min(36, (int)$jemsettings->recurrence_anticipation_year);
                            break;
                        case 6:
                            $anticipation    = min(36, (int)min(36, (int)$jemsettings->recurrence_anticipation_lastday));
                            break;
                        default:
                            $anticipation    = min(36, (int)$jemsettings->recurrence_anticipation_day);
                            break;
                    ...
                    {
                        $new_event = new \KoelmanLabs\Component\Planjeagenda\Administrator\Table\EventTable($db);
                        ...
                        if ($new_event->store())
                        {
                            ...
                            // Copy catid from the original event to the new recurrence instance
                            $query = 'UPDATE #__pja_events SET catid = '
                                   . '(SELECT catid FROM (SELECT catid FROM #__pja_events WHERE id = ' . (int)$ref_event->id . ') AS src) '
                                   . 'WHERE id = ' . (int)$new_event->id;
                            $db->setQuery($query);
                ...
                if ($jemsettings->oldevent == 1) {
                    $query = 'DELETE FROM #__pja_events WHERE dates > 0 AND '
                ...
                if ($jemsettings->oldevent == 2) {
                    $query = 'UPDATE #__pja_events SET published = 2 WHERE dates > 0 AND '
                           .' DATE_SUB(NOW(), INTERVAL '.(int)$jemsettings->minus.' DAY) > (IF (enddates IS NOT NULL, enddates, dates)) '
                           .' AND published = 1';
                    $db->SetQuery($query);
                    $db->execute();
                ...
                if ($jemsettings->oldevent == 4) {
                    $query = 'UPDATE #__pja_events SET published = 0 WHERE dates > 0 AND '
                           .' DATE_SUB(NOW(), INTERVAL '.(int)$jemsettings->minus.' DAY) > (IF (enddates IS NOT NULL, enddates, dates)) '
                           .' AND published = 1';
                    $db->SetQuery($query);
                    $db->execute();
                ...
                $query = 'DELETE FROM #__pja_register WHERE event NOT IN (SELECT id FROM #__pja_events)';
                $db->SetQuery($query);
                $db->execute();

                // Set timestamp of last cleanup
                PlanjeagendaConfig::getInstance()->set('lastupdate', $now);
                // Clear timestamp of running cleanup
                PlanjeagendaConfig::getInstance()->set('runningupdate', 0);
            }

            PlanjeagendaHelper::addLogEntry('finished.', __METHOD__);
        }
    }
     ...
     */
    static public function calculate_recurrence_legacy($recurrence_row)
    {
        ...
        $date_array = PlanjeagendaHelper::generate_date($recurrence_row['dates'], $recurrence_row['enddates']);
        ...
        switch($recurrence_type) {
            ...
            case "4": // weekday
                ...
                $selected = PlanjeagendaHelper::convert2CharsDaysToInt(explode(',', $recurrence_row['recurrence_byday']), 0);
                ...
                if (count($selected) == 0)
                {
                    ...
                    Factory::getApplication()->enqueueMessage(Text::_('com_planjeagenda_WRONG_EVENTRECURRENCE_WEEKDAY'), 'warning');
        ...
        if ($start_day < $date_array["unixtime"]) {
            throw new Exception(Text::_('com_planjeagenda_RECURRENCE_DATE_GENERATION_ERROR'), 500);
        }
    ...
    {
        // Delegate to RecurrenceHelper::calculateNext.
        // RecurrenceService was never created — this was a wrong namespace reference.
        static $loaded = false;
        if (!$loaded) {
            $file = JPATH_ADMINISTRATOR . '/components/com_planjeagenda/src/Helper/RecurrenceHelper.php';
            if (file_exists($file)) {
                require_once $file;
            }
            $loaded = true;
        ...
        $result = \KoelmanLabs\Component\Planjeagenda\Administrator\Helper\RecurrenceHelper::calculateNext(
            (array) $recurrence_row
        );

        // calculateNext returns false when no next occurrence can be calculated
        return $result !== false ? $result : $recurrence_row;
    }
    ...
    static public function dissolve_recurrence($first_id)
    {
        ...
        try {
            $db = Factory::getContainer()->get('DatabaseDriver');
            $db->setQuery('UPDATE #__pja_events'
    ...
    {
        $validStardate = PlanjeagendaHelper::isValidDate($startdate);
        $validEnddate = PlanjeagendaHelper::isValidDate($enddate);
    ...
    static public function getAccesslevelOptions($ownonly = false, $disabledLevels = false)
    {
        ...
        //PlanjeagendaHelper::addLogEntry('AccessLevel query: ' . $query, __METHOD__);

        $db->setQuery($query);
        $groups = $db->loadObjectList();

        //PlanjeagendaHelper::addLogEntry('result: ' . print_r($groups, true), __METHOD__);
                ...
                if($waitreg->places <= $placesavailable)
                {
                    ...
                    if ($db->execute() === false)
                    {
                        Factory::getApplication()->enqueueMessage(
                            Text::_(
                                'com_planjeagenda_FAILED_BUMPING_USERS_FROM_WAITING_TO_CONFIRMED_LIST'
                            ) . ': ' . $db->getErrorMsg(),
                            'warning'
                        );
                    }
    ...
    {
        require_once JPATH_SITE.'/components/com_planjeagenda/classes/ical.class.php';
        $timezone_name = PlanjeagendaHelper::getTimeZoneName();
    ...
    {
        PlanjeagendaDebug::log('icalAddEvent', 'event=' . ($event->title ?? '') . ' id=' . ($event->id ?? ''));
        $jemsettings   = PlanjeagendaHelper::config();
        $timezone_name = PlanjeagendaHelper::getTimeZoneName();
        ...
        // no start date...
        $validdate = PlanjeagendaHelper::isValidDate($event->dates);

        if (!$event->dates || !$validdate) {
            return false;
        }

        // make end date same as start date if not set or invalid
        if (!$event->enddates || $event->enddates === '0000-00-00') {
            $event->enddates = $event->dates;
        }
        // If enddates is before dates (data error), clamp to dates
        ...
        if (!preg_match('/([0-9]{4})-([0-9]{1,2})-([0-9]{1,2})/', $event->dates, $start_date)) {
            throw new Exception(Text::_('com_planjeagenda_ICAL_EXPORT_WRONG_STARTDATE_FORMAT'), 0);
        }
        ...
        if (!$event->times) // all day !
        {
            ...
            if (!preg_match('/([0-9]{4})-([0-9]{1,2})-([0-9]{1,2})/', $event->enddates, $end_date)) {
                throw new Exception(Text::_('com_planjeagenda_ICAL_EXPORT_WRONG_ENDDATE_FORMAT'), 0);
            }
        ...
        else // not all day events, there is a start time
        {
            if (!preg_match('/([0-9]{2}):([0-9]{2}):([0-9]{2})/', $event->times, $start_time)) {
                throw new Exception(Text::_('com_planjeagenda_ICAL_EXPORT_WRONG_STARTTIME_FORMAT'), 0);
            }
        ...
        $description .= Text::_('com_planjeagenda_CATEGORY').': '.implode(', ', $categories).'\\n';

        $link = $uri->root().PlanjeagendaHelperRoute::getEventRoute($event->slug);
        $link = Route::_($link);
        $description .= Text::_('com_planjeagenda_ICS_LINK').': '.$link.'\\n';
    ...
    {
        // Always use the responsive (card) layout — it's the only maintained layout.
        // The old table-based layout has been removed in favour of the card design.
        return 'responsive';
    }
    ...
    static public function loadCss($css)
    {
        ...
        if ($settings->get('css_' . $css . '_usecustom', '0')) {
            ...
            if ($file) {
                ...
                $is_file = is_file(JPATH_SITE . '/media/com_planjeagenda/css/custom/' . $file);
            ...
            if ($is_file) {
                ...
                $css = $document->addStyleSheet($url.'media/com_planjeagenda/css/custom/' . $file);
            } else {
                # unfortunately we don't have a valid file so we're looking at the default
                // $files = HTMLHelper::_('stylesheet', 'com_planjeagenda/' . $css . $suffix . '.css', array(), true, true);
                $files = $document->addStyleSheet($url.'media/com_planjeagenda/css/custom/' . $css . $suffix . '.css');
                if (!empty($files)) {
                    # we have to call this stupid function twice; no other way to know if something was loaded
                    // $css = HTMLHelper::_('stylesheet', 'com_planjeagenda/' . $css . $suffix . '.css', array(), true);
                    $css = $document->addStyleSheet($url.'media/com_planjeagenda/css/custom/' . $css . $suffix . '.css');

                } else {
                    # no css for layout style configured, so use the default css
                    // $css = HTMLHelper::_('stylesheet', 'com_planjeagenda/' . $css . '.css', array(), true);
                    $css = $document->addStyleSheet($url.'media/com_planjeagenda/css/custom/'. $css. '.css');
            ...
            // $files = HTMLHelper::_('stylesheet', 'com_planjeagenda/' . $css . $suffix . '.css', array(), true, true);
            $files = $document->addStyleSheet($url.'media/com_planjeagenda/css/' . $css . $suffix . '.css');

            if (!empty($files)) {
                # we have to call this stupid function twice; no other way to know if something was loaded
                // $css = HTMLHelper::_('stylesheet', 'com_planjeagenda/' . $css . $suffix . '.css', array(), true);
                $css = $document->addStyleSheet($url.'media/com_planjeagenda/css/' . $css . $suffix . '.css');

            } else {
                # no css for layout style configured, so use the default css
                // $css = HTMLHelper::_('stylesheet', 'com_planjeagenda/' . $css . '.css', array(), true);
                $css = $document->addStyleSheet($url.'media/com_planjeagenda/css/'. $css. '.css');
    ...
    public static function loadModuleStyleSheet($module, $css)
    {
        ...
        //Error no css file found
            ...
            PlanjeagendaHelper::addLogEntry("Warning: The file " . $filestyle . " couldn't be found.", __METHOD__);
        }
    }
    ...
    static public function loadIconFont()
    {
        ...
        if ($jemsettings->useiconfont == 1) {
            ...
            # Note: css files must be stored on /media/com_planjeagenda/css/ to be conform to Joomla and also allow template overrides.
            HTMLHelper::_('stylesheet', 'media/vendor/fontawesome-free/css/font-awesome.min.css', array(), true);
            HTMLHelper::_('stylesheet', 'com_planjeagenda/css/klevents-icon-font.css', array(), true);
        }
    }
