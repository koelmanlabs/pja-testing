...
<div id="klevents" class="jlcalendar jem_calendar<?php echo $this->pageclass_sfx;?>">
    <div class="buttons">
        ...
        echo \PlanjeagendaOutput::createButtonBar($this->getName().'-cal', $this->permissions, $btn_params);
        ?>
    </div>
    ...
    foreach ($this->rows as $row) :
        ...
        if ($countperday[$year.$month.$day] == $limit+1) {
            $var1a = \Route::_('index.php?option=com_planjeagenda&view=day&id='.$year.$month.$day.'&catid='.$this->catid);
    ...
    if (!$this->print && $this->params->get('show_addevent_icon', 0) && !empty($this->permissions->canAddEvent)) {
        $html = \PlanjeagendaOutput::prepareAddEventButton('catid='.$this->catid);
