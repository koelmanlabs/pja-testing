...
<div id="klevents" class="jem_venue<?php echo $this->pageclass_sfx . ' venue_id' . $this->venue->id; ?>" itemscope="itemscope" itemtype="https://schema.org/Place">
    <div class="buttons">
        <?php
    ...
    <h1 class="componentheading">
        ...
        <?php
        echo \PlanjeagendaOutput::editbutton($this->venue, $this->params, NULL, $this->permissions->canEditVenue, 'venue');
        ?>
    </h1>
    <?php endif; ?>
  ...
  <?php if ($this->escape($this->params->get('page_heading')) != $this->escape($this->venue->title)) : ?>
    <?php if ($this->escape($this->params->get('show_page_heading', 1))) : ?>
      <h2 class="klevents-venue-title">
        <?php echo $this->escape($this->venue->title);?>
      </h2>
    <?php else : ?>
      <h1 class="klevents-venue-title">
        <?php echo $this->escape($this->venue->title);?>
      </h1>
    <?php endif; ?>
    <?php endif; ?>
    ...
    <!--Venue-->
    <h2 class="klevents">
        <?php echo \Text::_('com_planjeagenda_VENUE'); ?>
    </h2>

    <?php echo \PlanjeagendaOutput::flyer($this->venue, $this->limage, 'venue'); ?>

    <?php if (($this->settings->get('global_show_detlinkvenue', 1)) && (!empty($this->venue->url))) : ?>
        <dl class="location">
            <dt class="venue"><?php echo \Text::_('com_planjeagenda_WEBSITE'); ?>:</dt>
            <dd class="venue">
    ...
    <?php if ($this->settings->get('global_show_detailsadress', 1)) : ?>
        <dl class="location floattext" itemprop="address" itemscope itemtype="https://schema.org/PostalAddress">
            ...
            if ($this->settings->get('global_show_mapserv') == 1 || $this->settings->get('global_show_mapserv') == 4) {
                echo \PlanjeagendaOutput::mapicon($this->venue, null, $this->settings);
            }
            endif; ?>
        ...
        <?php if ($this->settings->get('global_show_mapserv') == 2 || $this->settings->get('global_show_mapserv') == 5) : ?>
            <div class="klevents-map">
                <?php echo \PlanjeagendaOutput::mapicon($this->venue, null, $this->settings); ?>
            </div>
        <?php endif;?>
    ...
    <?php if ($this->settings->get('global_show_locdescription', 1) && $this->venuedescription != '' &&
        ...
        <h2 class="description"><?php echo \Text::_('com_planjeagenda_VENUE_DESCRIPTION'); ?></h2>
    ...
    <?php if ($this->settings->get('global_show_listevents', 1)) : ?>
        ...
        <div id="iCal" class="iCal">
            <?php echo \PlanjeagendaOutput::icalbutton($this->venue->id, 'venue'); ?>
         </div>
    <?php endif; ?>
