...
namespace KoelmanLabs\Component\Planjeagenda\Site\View\Event;
...
class HtmlView extends BaseHtmlView
{
    ...
    public function display($tpl = null)
    {
        ...
        $this->params      = ($app->isClient('administrator') ? \Joomla\CMS\Component\ComponentHelper::getParams('com_planjeagenda') : $app->getParams('com_planjeagenda'));
        ...
        // Guard: if event not found, show 404
        if (empty($this->item) || empty($this->item->id)) {
            throw new \Joomla\CMS\Exception\ExceptionHandler();
        }
        ...
        //\PlanjeagendaHelper::addLogEntry("Attendees:\n" . print_r($this->registers, true), __METHOD__);
        ...
        if ($useMenuItemParams) {
            ...
            $pathway->addItem($pagetitle, Route::_(RouteHelper::getEventRoute($item->slug)));
        ...
        $results = Factory::getApplication()->triggerEvent('onContentPrepare', array ('com_planjeagenda.event', &$item, &$this->params, $offset));

        $item->event = new \stdClass();
        $results = Factory::getApplication()->triggerEvent('onContentAfterTitle', array('com_planjeagenda.event', &$item, &$this->params, $offset));
        $item->event->afterDisplayTitle = trim(implode("\n", $results));

        $results = Factory::getApplication()->triggerEvent('onContentBeforeDisplay', array('com_planjeagenda.event', &$item, &$this->params, $offset));
        $item->event->beforeDisplayContent = trim(implode("\n", $results));

        $results = Factory::getApplication()->triggerEvent('onContentAfterDisplay', array('com_planjeagenda.event', &$item, &$this->params, $offset));
        ...
        $results = Factory::getApplication()->triggerEvent('onContentPrepare', array ('com_planjeagenda.event', &$tempVenue, &$this->params, $offset));
     ...
     /*   $this->print_link = Route::_(\PlanjeagendaHelperRoute::getRoute($item->slug).'&print=1&tmpl=component'); */

        // Get images
        $this->dimage = \PlanjeagendaImage::flyercreator($item->datimage, 'event');
        $this->limage = \PlanjeagendaImage::flyercreator($item->locimage, 'venue');
        ...
        $permissions->canEditEvent = $user->authorise('core.edit', 'com_planjeagenda');
        $permissions->canPublishEvent = $user->authorise('core.edit.state', 'com_planjeagenda');

        // Check if user can edit the venue
        $permissions->canEditVenue = $user->authorise('core.edit', 'com_planjeagenda');
        $permissions->canPublishVenue = $user->authorise('core.edit.state', 'com_planjeagenda');
        ...
        $permissions->canEditAttendees = $user->authorise('core.edit', 'com_planjeagenda');
        //suggestion by M59S to allow groupmembers too see line 230/231 too
        $edit_att->canEditAttendees = $user->authorise('core.edit', 'com_planjeagenda');
        ...
        if ($this->print) {
            ...
            PluginHelper::importPlugin('klevents', 'comments');
            $results = Factory::getApplication()->triggerEvent('onEventEnd', array($item->did, $this->escape($item->title)));
            $item->pluginevent->onEventEnd = trim(implode("\n", $results));
        }

        // create flag
        if ($item->country) {
            $item->countryimg = PlanjeagendaHelperCountries::getCountryFlag($item->country);
        }
        ...
        switch ($keyword)
        {
            ...
            case 'endtimes':
                if (isset($row->$keyword)) {
                    $content = OutputHelper::formattime($row->$keyword);
                }
                break;
            ...
            case 'enddates':
                if (isset($row->$keyword)) {
                    $content = OutputHelper::formatdate($row->$keyword);
                }
                break;
    ...
    protected function _prepareDocument()
    {
        ...
        */
        ...
        /*
            ...
            if ($menu && ($menu->query['option'] != 'com_planjeagenda' || $menu->query['view'] != 'event' || $id != $this->item->id)) {
                ...
                while ($category && ($menu->query['option'] != 'com_planjeagenda' || $menu->query['view'] == 'event'
                        || $id != $category->id) && $category->id > 1) {
                    $path[] = array('title' => $category->catname, 'link' => \PlanjeagendaHelperRoute::getCategoryRoute($category->id));
        ...
        */
        ...
        $showDateInTitle = $this->item->params->get('show_date_in_title', $this->jemsettings->show_date_in_title ?? 0);
            if ($showDateInTitle) {
                ...
                if (!empty($this->item->dates)) {
                    $startDate = \PlanjeagendaOutput::formatdate($this->item->dates);
                    $title .= ', ' . $startDate;
                    // add end date to browser title, if availaböe
                    if (!empty($this->item->enddates) && $this->item->enddates != $this->item->dates) {
                        $endDate = \PlanjeagendaOutput::formatdate($this->item->enddates);
                        $title .= ' - ' . $endDate;
                    }
                } else {
                    $title .= ', ' . Text::_('com_planjeagenda_OPEN_DATE');
                }
            }
        ...
        if ($app->get('MetaAuthor') == '1') {
            $document->setMetaData('author', $this->item->author);
        }
        ...
        if (!empty($this->item->page_title)) {
            ...
            $document->setTitle($this->item->page_title . ' - '
