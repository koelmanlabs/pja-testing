...
<dl class="klevents-dl">
  ...
  <dd>
    <div class="klevents-row klevents-nowrap klevents-justify-start"><?php echo \Text::_('com_planjeagenda_SEARCH_FROM'); ?>&nbsp;<?php echo $this->lists['date_from'];?></div>
    <div class="klevents-row klevents-nowrap klevents-justify-start"><?php echo \Text::_('com_planjeagenda_SEARCH_TO'); ?>&nbsp;<?php echo $this->lists['date_to'];?></div>
  </dd>
  <dt>
...
<ul class="eventlist">
  ...
  <?php else : ?>
      ...
      <?php if (!empty($row->featured)) :   ?>
        <li class="klevents-event klevents-list-row klevents-small-list klevents-featured event_id<?php echo $row->id . ' ' . $this->params->get('pageclass_sfx'); ?>" itemscope="itemscope" itemtype="https://schema.org/Event">
      <?php else : ?>
        <li class="klevents-event klevents-list-row klevents-small-list event_id<?php echo $row->id . ' klevents-odd' . ($row->odd +1) . $this->params->get('pageclass_sfx'); ?>" itemscope="itemscope" itemtype="https://schema.org/Event">
      <?php endif; ?>
            ...
            <?php if ($this->jemsettings->showstate == 1) : ?>
              <?php if (!empty($row->state)) : ?>
                <div class="klevents-event-info-small klevents-event-state" title="<?php echo \Text::_('COM_PLANJEAGENDA_VENUE_PROVINCE_LABEL').': '.$this->escape($row->state); ?>">
