...
use KoelmanLabs\Component\Planjeagenda\Site\Helper\PlanjeagendaFactory;
...
class EventslistModel extends ListModel
{
    ...
    protected function populateState($ordering = null, $direction = null)
    {
        ...
        $year = $app->input->getInt('yearID', date('Y'));
        $month = $app->input->getInt('monthID', date('m'));
        
        $itemid = $input->getInt('Itemid', 0);
        
      $stateKey = 'com_planjeagenda.eventslist.' . $itemid;
        ...
        /*
        * --------------------------------------------------
        * PAGINATION (ALWAYS APPLY - HTML + ICS + RAW)
        * --------------------------------------------------
        */
        $limit = $app->getUserStateFromRequest(
            $stateKey . '.limit',
            'limit',
            (int) $params->get('display_num', 10),
            'int'
            );
        ...
        $start = $limit ? (int)(floor($start / $limit) * $limit) : 0;
        
        $this->setState('list.limit', $limit);
        $this->setState('list.start', $start);

        # Search - variables
        $search     = $app->getUserStateFromRequest('com_planjeagenda.eventslist.' . $itemid . '.filter_search', 'filter_search', '', 'string');
        $this->setState('filter.filter_search', $search); // must be escaped later

        // If filter_search comes from URL (not form), ensure filter_type is also read from URL
        // so tag links (?filter_search=Muziek&filter_type=1) work correctly
        if ($app->input->get('filter_search', '') !== '') {
            $urlFilterType = $app->input->getInt('filter_type', 0);
            if ($urlFilterType > 0) {
                $app->setUserState('com_planjeagenda.eventslist.' . $itemid . '.filter_type', $urlFilterType);
            }
        }

        $filtertype = $app->getUserStateFromRequest('com_planjeagenda.eventslist.' . $itemid . '.filter_type', 'filter_type', 0, 'int');
        $this->setState('filter.filter_type', $filtertype);

        $filtermonth = $app->getUserStateFromRequest('com_planjeagenda.eventslist.' . $itemid . '.filter_month', 'filter_month', 0, 'string');
        $this->setState('filter.filter_month', $filtermonth);

        // Region/province filter â€” set by the regio map module or filter bar
        $venueState = $app->getUserStateFromRequest('com_planjeagenda.eventslist.' . $itemid . '.filter_venue_state', 'filter_venue_state', '', 'string');
        if ($venueState !== '') {
            $this->setState('filter.venue_state', [$venueState]);
            $this->setState('filter.venue_state.mode', 0);
        }

        // Tag filter â€” filter_tag_id param from URL or form
        $filterTagId = $app->getUserStateFromRequest(
            'com_planjeagenda.eventslist.' . $itemid . '.filter_tag_id',
            'filter_tag_id', 0, 'int'
        );
        if ($filterTagId) {
            $this->setState('filter.tag_id', (int)$filterTagId);
        }

        // Venue text and city text filters â€” from expanded filter panel
        $filterVenueText = $app->getUserStateFromRequest('com_planjeagenda.eventslist.' . $itemid . '.filter_venue_text', 'filter_venue_text', '', 'string');
        $filterCityText  = $app->getUserStateFromRequest('com_planjeagenda.eventslist.' . $itemid . '.filter_city_text',  'filter_city_text',  '', 'string');
        if ($filterVenueText !== '') {
            $this->setState('filter.venue_text', $filterVenueText);
        }
        if ($filterCityText !== '') {
            $this->setState('filter.city_text', $filterCityText);
        }

        // Category filter â€” from filter bar dropdown
        $filterCatId = $app->getUserStateFromRequest('com_planjeagenda.eventslist.' . $itemid . '.filter_catid', 'filter_catid', 0, 'int');
        if ($filterCatId > 0) {
            $this->setState('filter.filter_catid', $filterCatId);
        }

        // Date range filters â€” from filter bar date pickers
        $db = \Joomla\CMS\Factory::getContainer()->get('DatabaseDriver');
        $filterDateFrom = $app->getUserStateFromRequest('com_planjeagenda.eventslist.' . $itemid . '.filter_date_from', 'filter_date_from', '', 'string');
        $filterDateTo   = $app->getUserStateFromRequest('com_planjeagenda.eventslist.' . $itemid . '.filter_date_to',   'filter_date_to',   '', 'string');
        if (!empty($filterDateFrom) && preg_match('/^\d{4}-\d{2}-\d{2}$/', $filterDateFrom)) {
            $this->setState('filter.calendar_from', '(COALESCE(a.enddates, a.dates) >= ' . $db->quote($filterDateFrom) . ')');
        }
        if (!empty($filterDateTo) && preg_match('/^\d{4}-\d{2}-\d{2}$/', $filterDateTo)) {
            $this->setState('filter.calendar_to', '(a.dates <= ' . $db->quote($filterDateTo) . ')');
        }

        # Search - Filter by setting menu
        $today = new Date('now', \Joomla\CMS\Factory::getConfig()->get('offset', 'UTC'));
        ...
        if ($filterDaysBefore > 0) {
            ...
            // Use range comparison instead of DATEDIFF() so idx_dates index can be used.
            // DATEDIFF wraps the column in a function, preventing index use.
            $whereFrom = ' (COALESCE(a.enddates, a.dates) >= "' . $dateFromValue . '")';
        }
        if (!empty($whereFrom)) {
        ...
        $filter_order = $app->getUserStateFromRequest('com_planjeagenda.eventslist.' . $itemid . '.filter_order', 'filter_order', 'a.dates', 'cmd');
        ...
        if (empty($app->input->get('filter_type')) && $tableInitialorderby) {
            ...
            $filter_order = $app->getUserStateFromRequest('com_planjeagenda.eventslist.' . $itemid . '.filter_order', 'filter_order', $tableInitialorderby, 'cmd');
        ...
        $filter_order_Dir = $app->getUserStateFromRequest('com_planjeagenda.eventslist.' . $itemid . '.filter_order_Dir', 'filter_order_Dir', $filter_order_DirDefault, 'word');

        $default_order_Dir = ($task == 'archive') ? 'DESC' : 'ASC';

        $allowed_cols = ['a.dates','a.title','a.created','a.hits','l.venue','l.city','l.state','c.catname'];
        $filter_order = PlanjeagendaHelper::sanitizeOrderCol($filter_order, $allowed_cols, 'a.dates');
        $allowed_order = ['a.dates','a.title','l.venue','l.city','l.state','c.catname','a.created','a.hits','a.featured'];
        $filter_order = PlanjeagendaHelper::sanitizeOrderCol($filter_order, $allowed_order, 'a.dates');
        $allowed_order = ['a.dates','a.title','l.venue','l.city','l.state','c.catname','a.created','a.hits','a.featured'];
        $filter_order = PlanjeagendaHelper::sanitizeOrderCol($filter_order, $allowed_order, 'a.dates');
        ...
        $this->setState('calendar.year', $year);
        $this->setState('calendar.month', $month);
    ...
    protected function getStoreId($id = '')
    {
        ...
        $id .= ':' . serialize($this->getState('filter.venue_state'));
        $id .= ':' . $this->getState('filter.venue_state.mode');
        $id .= ':' . $this->getState('filter.filter_search');
        $id .= ':' . $this->getState('filter.filter_type');
        $id .= ':' . $this->getState('filter.venue_text');
        $id .= ':' . $this->getState('filter.city_text');
        $id .= ':' . $this->getState('filter.tag_id');
        $id .= ':' . $this->getState('filter.filter_catid');
    ...
    protected function getListQuery()
    {
        ...
        $query->join('LEFT', '#__pja_venues AS l ON l.id = a.locid');
        
        // Koppel de locatie aan het dorp
        $query->join('LEFT', $db->quoteName('#__pja_villages', 'vi') . ' ON ' . $db->quoteName('vi.id') . ' = ' . $db->quoteName('l.village_id'));

        # Country
        $query->select(array('ct.name AS countryname'));
        $query->join('LEFT', '#__pja_countries AS ct ON ct.iso2 = l.country');
        ...
        $query->join('LEFT', '#__pja_categories AS c ON c.id = a.catid');
        ...
        // Haal de geselecteerde filters op uit de request/state
        $municipalityId = $app->getUserStateFromRequest('com_planjeagenda.eventslist.filter.municipality', 'filter_municipality', 0, 'int');
        $villageId      = $app->getUserStateFromRequest('com_planjeagenda.eventslist.filter.village', 'filter_village', 0, 'int');
        
        // Als er een specifiek dorp is gekozen, filter daarop (meest specifiek)
        if ($villageId > 0)
        {
            $query->where($db->quoteName('v.village_id') . ' = ' . (int) $villageId);
        }
        // Als er géén dorp is gekozen, maar wel een gemeente, filter op alle dorpen in die gemeente
        ...
        $currentDate  = (new Date('now', \Joomla\CMS\Factory::getConfig()->get('offset', 'UTC')))->format($db->getDateFormat(), true);
        ...
        if($opendates != 2 && $ispublished <2) {
            ...
            if ($calendar_from != '') {
                ...
                # Si tambiÃ©n hay un calendar_to, lo aÃ±adimos
                if ($calendar_to != '') {
                ...
                if($published<2) {
                    if (empty($jubilee_filter) || !$jubilee_show_past) {
                        if ($cal_month) {
                            ...
                            // Range comparison lets MySQL use idx_dates â€” DATEDIFF() would disable it.
                            $where_from = ' (COALESCE(a.enddates, a.dates) >= ' . $db->quote($filter_date_from) . ' ' . $opendates_query;
                            $query->where($where_from);
                            $this->setState('filter.calendar_from', $where_from);

                            // Check if event STARTS before or on the end date
                            $where_to = ' (a.dates <= ' . $db->quote($filter_date_to) . ' ' . $opendates_query;
        ...
        // Tag filter via Joomla's contentitem_tag_map
        $filterTagId = (int) $this->getState('filter.tag_id', 0);
        if ($filterTagId) {
            $query->join(
                'INNER',
                '#__contentitem_tag_map AS tagmap'
                . ' ON tagmap.content_item_id = a.id'
                . ' AND tagmap.type_alias = ' . $db->quote('com_planjeagenda.event')
                . ' AND tagmap.tag_id = ' . $filterTagId
            );
        }
        ...
        // Venue text search (from expanded filter panel)
        $filterVenueText = $this->getState('filter.venue_text', '');
        if ($filterVenueText !== '') {
            $query->where('l.venue LIKE ' . $db->quote('%' . $db->escape($filterVenueText) . '%'));
        }

        // City text search (from expanded filter panel)
        $filterCityText = $this->getState('filter.city_text', '');
        if ($filterCityText !== '') {
            $query->where('l.city LIKE ' . $db->quote('%' . $db->escape($filterCityText) . '%'));
        }
        ...
        if (!empty($venueState)) {
            // Normalise: accept both array (from regio module) and comma string (legacy)
            if (is_array($venueState)) {
                $venueState = array_filter(array_map('trim', $venueState));
            } else {
                $venueState = array_filter(array_map('trim', explode(',', $venueState)));
            }
        ...
        if (!empty($search)) {
            if (stripos($search, 'id:') === 0) {
                ...
                // Always apply search filter when present in URL â€”
                // global_show_filter only controls UI visibility, not filtering
                switch ($filter) {
                    case 2:
                    ...
                    case 5:
                        ...
                        break;
                    default: // case 1 + anything else = search by title
        ...
        $fcStart = $this->getState('filter.start_date');
        $fcEnd   = $this->getState('filter.end_date');
        
        if (!empty($fcStart) && !empty($fcEnd)) {
            // Wis eventuele oude, conflicterende JEM-datumfilters die vaststaan op de huidige maand
            $query->clear('where');
            
            // Bouw de basisvoorwaarden opnieuw op (gepubliceerd, niet gearchiveerd, etc.)
            $query->where($db->quoteName('a.published') . ' = 1');
            
            // Voeg de flexibele datum-overlapping toe voor FullCalendar
            // Dit pakt events die starten óf eindigen binnen het actieve kalendervenster
            $query->where('('
                . $db->quoteName('a.dates') . ' <= ' . $db->quote($fcEnd) . ' AND ('
                . $db->quoteName('a.enddates') . ' >= ' . $db->quote($fcStart)
    ...
    public function getItems()
    {
        ...
        foreach ($items as $index => $item)
        {
            ...
            if (empty($item->categories)) {
                ...
                $jemsettings2 = PlanjeagendaHelper::config();
                $eventowner2 = isset($jemsettings2->eventowner) ? (int)$jemsettings2->eventowner : 0;
                $canEdit2 = $user->authorise('core.edit', 'com_planjeagenda')
                    || ($eventowner2 && $user->id && $user->id == ($item->created_by ?? 0));
                $item->params->set('access-edit', $canEdit2);
            }
        } // foreach

        if ($items) {
            /*$items =*/ PlanjeagendaHelper::getAttendeesNumbers($items);
    ...
    {
        // Single-category: fetch the event's primary category directly via a.catid
        $db     = $this->getDatabase();
        $user   = \Joomla\CMS\Factory::getApplication()->getIdentity();
        $levels = implode(',', $user->getAuthorisedViewLevels());

        $db->setQuery(
            'SELECT c.id, c.catname, c.alias, c.color, c.access, c.published, c.path,'
            . ' 1 AS user_has_access_category,'
            . " CONCAT(c.id, ':', c.alias) AS catslug"
    ...
    protected function _populatePublishState($task)
    {
        ...
        if ($task == 'archive') {
            ...
            $show_unpublished = $user->authorise('core.edit', 'com_planjeagenda') || $user->authorise('core.edit.state', 'com_planjeagenda');
            if ($show_unpublished) {
                ...
                if (($userId !== 0) && ($jemsettings->eventedit == -1)) {
                    $jemgroups[0] = true; // we need key 0 to get unpublished events not attached to any klevents group
                }
                // user permitted on that klevents groups
    ...
    protected function _getPublishWhere($tbl = 'a')
    {
        ...
        {
            // Is user member of klevents groups allowing to see unpublished events?
            ...
            if (is_array($unpublished_on_groups) && !empty($unpublished_on_groups)) {
                ...
                $where_pub[] = '(' . $tbl . 'published = ' . $unpublished . ' AND '
                    . $tbl . 'id NOT IN (SELECT rel3.itemid FROM #__pja_categories as c3 '
                    . '                   INNER JOIN #__pja_cats_event_relations as rel3 '
    ...
    public function setDate($timestamp): void
    {
    ...
    /**
     * Haal alle dorpen op voor een specifieke gemeente
     */
    public function getVillagesByWithMunicipality($municipalityId)
    {
        $db    = $this->getDbo();
        $query = $db->getQuery(true);
        ...
        ->where($db->quoteName('municipality_id') . ' = ' . (int) $municipalityId)
