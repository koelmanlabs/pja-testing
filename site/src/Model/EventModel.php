...
class EventModel extends ItemModel
{
     ...
     * @var string
     */
    protected $_context = 'com_planjeagenda.event';
    ...
    protected function populateState()
    {
        ...
        // Load the parameters.
        $params = $app->isClient('administrator') ? ComponentHelper::getParams('com_planjeagenda') : $app->getParams('com_planjeagenda');
    ...
    public function getItem($pk = null)
    {
        ...
        if (!isset($this->_item[$pk]))
        {
            try
            {
                ...
                $query->join('LEFT', '#__pja_venues AS l ON a.locid = l.id');

                # Join over the category tables
                $query->join('LEFT', '#__pja_categories AS c2 ON c2.id = a.catid');
                $query->join('LEFT', '#__pja_categories AS c ON c.id = c.id');
                ...
                */
                ...
                if (empty($data)) {
                    throw new \Exception(Text::_('COM_PLANJEAGENDA_EVENT_ERROR_EVENT_NOT_FOUND'), 404);
                }

                # Convert parameter fields to objects.
                $registry = new Registry;
                $registry->loadString($data->attribs);
                $data->params = PlanjeagendaHelper::globalattribs(); // returns Registry object
                ...
                $access_edit = $user->authorise('core.edit', 'com_planjeagenda');
                $access_view = (($data->published == 1) || ($data->published == 2) ||          // published and archived event
                    ...
                    $user->authorise('core.edit.state', 'com_planjeagenda')); // all for publishers
...
/*        $this->_item[$pk]->attachments = \PlanjeagendaAttachment::getAttachments('event' . $this->_item[$pk]->did); */

        # Get venue attachments
 /*       $this->_item[$pk]->vattachments = \PlanjeagendaAttachment::getAttachments('venue' . $this->_item[$pk]->locid); */
    ...
    public function getListRecurrenceEventsbyId ($id, $pk, $datetimeFrom, $iduser=null, $status=null)
    {
        ...
        try
        {
            ...
            if($iduser) {
                $query->join('LEFT', '#__pja_register AS r ON r.event = a.id');
            ...
            $data[0]->params = PlanjeagendaHelper::globalattribs(); // returns Registry object
    ...
    public function hit($pk = 0)
    {
        ...
        if ($hitcount) {
            ...
            $db->setQuery('UPDATE #__pja_events' . ' SET hits = hits + 1' . ' WHERE id = ' . (int) $pk);
    ...
    public function getCategories($id = 0)
    {
        ...
        $query->from('#__pja_categories as c');
        $query->join('LEFT', '#__pja_events AS ae3 ON ae3.catid = c.id');

        $query->select(array('a.id AS multi'));
        $query->join('LEFT','#__pja_events AS a ON a.id = a.id');
        ...
        //            . ' LEFT JOIN #__pja_groupmembers AS g ON g.group_id = gr.id'
        //            . ' WHERE g.member = ' . (int) $user->id
    ...
    {
        \PlanjeagendaDebug::log('registration', "eventId=$eventId uid=$uid status=$status places=$places");
        //    $app = Factory::getApplication('site');
        //    $user = Factory::getApplication()->getIdentity();
        //    $jemsettings = PlanjeagendaHelper::config();
        ...
        if (empty($event)) {
            $errMsg = Text::_('com_planjeagenda_EVENT_ERROR_EVENT_NOT_FOUND') . ' [id: ' . $eventId  .']';
            return false;
        }
            ...
            if ($respectPlaces && ($event->maxplaces > 0)) {    // there is a max
                ...
                if ($event->booked >= $event->maxplaces) {
                    if (!$event->waitinglist) {
                        $this->setError(Text::_('com_planjeagenda_EVENT_FULL_NOTICE'));
                        return false;
                    }
                    $onwaiting = 1;
                }
        ...
        elseif ($status == 2) {
            if ($respectPlaces && !$event->waitinglist) {
                $errMsg = Text::_('com_planjeagenda_NO_WAITINGLIST');
                return false;
            }
            $onwaiting = 1;
            $status = 1;
        ...
        elseif ($respectPlaces && ($oldstat == 1) && ($status == -1) && !$event->unregistra) {
            $errMsg = Text::_('com_planjeagenda_ERROR_ANNULATION_NOT_ALLOWED') . ' [id: ' . $eventId  .']';
            return false;
        }
        ...
        try {
            if ($regid) {
                $obj->id = $regid;
                $this->_db->updateObject('#__pja_register', $obj, 'id');
                $result = $regid;
            } else {
                $this->_db->insertObject('#__pja_register', $obj);
                $result = $this->_db->insertid();
            }
        }
        catch (Exception $e) {
            // we have a unique user-event key so registering twice will fail
            $errMsg = Text::_(($e->getCode() == 1062) ? 'com_planjeagenda_ALREADY_REGISTERED' : 'com_planjeagenda_ERROR_REGISTRATION') . ' [id: ' . $eventId  .']';
            return false;
        }
    ...
    public function userregister()
    {
        ...
        foreach ($events as $e) {
            ...
            // IP
            $uip = $jemsettings->storeip ? PlanjeagendaHelper::retrieveIP() : false;

            $result = $this->_doRegister($e->id, $uid, $uip, $status, $places, $comment, $errMsg, $reg ? $reg->id : null);
            if (!$result) {
                $this->setError(Text::_('com_planjeagenda_ERROR_REGISTRATION') . ' [id: ' . $e->id . ']');
            } else {
                Factory::getApplication()->enqueueMessage(($status==1? Text::_('com_planjeagenda_REGISTERED_USER_IN_EVENT') : Text::_('com_planjeagenda_UNREGISTERED_USER_IN_EVENT')), 'info');
            }
        }
        return $result;
    }
    ...
    public function delreguser()
    {
        ...
        $query = 'DELETE FROM #__pja_register WHERE event = ' . $event . ' AND uid= ' . $userid;
        $this->_db->SetQuery($query);
