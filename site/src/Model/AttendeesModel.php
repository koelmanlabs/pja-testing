...
use KoelmanLabs\Component\Planjeagenda\Site\Model\AttendeeModel;
use Joomla\Utilities\ArrayHelper;

class AttendeesModel extends BaseDatabaseModel
{
    /**
    ...
    public function __construct()
    {
        ...
        if ($app->input->getInt('limitstart', null) === null) {
            $app->setUserState('com_planjeagenda.attendees.limitstart', 0);
        }

        $limit      = $app->getUserStateFromRequest( 'com_planjeagenda.attendees.limit', 'limit', $jemsettings->display_num, 'int');
    ...
    protected function _buildQuery()
    {
        ...
        $query = 'SELECT r.*, u.username, u.name, u.email, a.created_by, a.published,'
               ...
               . ' LEFT JOIN #__pja_events AS a ON r.event = a.id'
    ...
    protected function _buildContentOrderBy()
    {
        ...
        $filter_order     = $app->getUserStateFromRequest('com_planjeagenda.attendees.filter_order',     'filter_order',     'r.uregdate', 'cmd' );
        $filter_order_Dir = $app->getUserStateFromRequest('com_planjeagenda.attendees.filter_order_Dir', 'filter_order_Dir', 'ASC',        'word' );
    ...
    protected function _buildContentWhere()
    {
        ...
        $canEdit = $user->authorise('core.edit', 'com_planjeagenda'); // where cluase ensures user is the event owner
    ...
    public function getEvent()
    {
        if (empty($this->_event)) {
            $query = 'SELECT a.id, a.alias, a.title, a.dates, a.enddates, a.times, a.endtimes, a.maxplaces, a.maxbookeduser, a.minbookeduser, a.reservedplaces, a.waitinglist, a.requestanswer, a.seriesbooking, a.singlebooking,'
                   ...
                   . ' FROM #__pja_events AS a WHERE a.id = '.$this->_db->Quote($this->_id);
    ...
    public function remove($cid = array())
    {
        if (is_array($cid) && count($cid))
        {
            ...
            $query = 'DELETE FROM #__pja_register WHERE id IN ('. implode(',', $cid) .') ';
    ...
    public function getUsersPagination()
    {
        $jemsettings = PlanjeagendaHelper::config();
        $app         = Factory::getApplication();
        $limit       = $app->getUserStateFromRequest('com_planjeagenda.addusers.limit', 'limit', $jemsettings->display_num, 'int');
