...
<div id="klevents" class="jlcalendar jem_calendar<?php echo $this->pageclass_sfx;?>">
    ...
    foreach ($this->rows as $row) :
        ...
        if ($showtime) {
            $start = OutputHelper::formattime($row->times);
            $end = OutputHelper::formattime($row->endtimes);
        ...
        $detaillink = Route::_(RouteHelper::getEventRoute($row->slug));
        ...
        if ($showtime) {
            $start = OutputHelper::formattime($row->times,'',false);
            $end   = OutputHelper::formattime($row->endtimes,'',false);
        ...
        $eventdate = !empty($row->multistartdate) ? OutputHelper::formatdate($row->multistartdate) : OutputHelper::formatdate($row->dates);
        if (!empty($row->multienddate)) {
            $eventdate .= ' - ' . OutputHelper::formatdate($row->multienddate);
        } else if ($row->enddates && $row->dates < $row->enddates) {
            $eventdate .= ' - ' . OutputHelper::formatdate($row->enddates);
        }
        ...
        switch ($multi_mode) {
        case 1:  // first day
            $multidaydate .= OutputHelper::formatShortDateTime($row->dates, $row->times, $row->enddates, $row->endtimes, $showtime);
            $multidaydate .= OutputHelper::formatSchemaOrgDateTime($row->dates, $row->times, $row->enddates, $row->endtimes);
            break;
        case 2:  // middle day
            $multidaydate .= OutputHelper::formatShortDateTime($row->multistartdate, $row->times, $row->multienddate, $row->endtimes, $showtime);
            $multidaydate .= OutputHelper::formatSchemaOrgDateTime($row->multistartdate, $row->times, $row->multienddate, $row->endtimes);
            break;
        case 3:  // last day
            $multidaydate .= OutputHelper::formatShortDateTime($row->multistartdate, $row->times, $row->multienddate, $row->endtimes, $showtime);
            $multidaydate .= OutputHelper::formatSchemaOrgDateTime($row->multistartdate, $row->times, $row->multienddate, $row->endtimes);
        ...
        if (!$this->print) {
            ...
            if ($this->params->get('show_editevent_icon', 0) && $row->params->get('access-edit', false)) {
                $btns[] = OutputHelper::editbutton($row, null, null, true, 'editevent');
            }
            if ($this->params->get('show_copyevent_icon', 0) && $this->permissions->canAddEvent) {
                $btns[] = OutputHelper::copybutton($row, null, null, true, 'editevent');
            }
            if (!empty($btns)) {
