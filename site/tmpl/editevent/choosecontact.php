...
<div id="klevents" class="jem_select_contact">
    ...
    <form action="<?php echo \Route::_('index.php?option=com_planjeagenda&view=editevent&layout=choosecontact&tmpl=component&function='.$this->escape($function).'&'.Session::getFormToken().'=1'); ?>" method="post" name="adminForm" id="adminForm">
        ...
        <table class="eventtable table table-striped" style="width:100%" summary="klevents">
            <thead>
                <tr>
                    <th style="width: 7px" class="sectiontableheader"><?php echo \Text::_('com_planjeagenda_NUM'); ?></th>
                    <th style="text-align: left;" class="sectiontableheader"><?php echo \HTMLHelper::_('grid.sort', 'com_planjeagenda_NAME', 'con.name', $this->lists['order_Dir'], $this->lists['order'] ); ?></th>
                    <?php if (0) : /* removed because it maybe forbidden to show */ ?>
                        <th style="text-align: left;" class="sectiontableheader"><?php echo \HTMLHelper::_('grid.sort', 'com_planjeagenda_ADDRESS', 'con.address', $this->lists['order_Dir'], $this->lists['order'] ); ?></th>
                    <?php endif; ?>
                    <th style="text-align: left;" class="sectiontableheader"><?php echo \HTMLHelper::_('grid.sort', 'com_planjeagenda_CITY', 'con.suburb', $this->lists['order_Dir'], $this->lists['order'] ); ?></th>
                    <th style="text-align: left;" class="sectiontableheader"><?php echo \HTMLHelper::_('grid.sort', 'com_planjeagenda_STATE', 'con.state', $this->lists['order_Dir'], $this->lists['order'] ); ?></th>
                    <?php if (0) : /* removed because it maybe forbidden to show */ ?>
                        <th style="text-align: left;" class="sectiontableheader"><?php echo \Text::_('com_planjeagenda_EMAIL'); ?></th>
                        <th style="text-align: left;" class="sectiontableheader"><?php echo \Text::_('com_planjeagenda_TELEPHONE'); ?></th>
                    <?php endif; ?>
                </tr>
            </thead>
            <tbody>
                <?php if (empty($this->rows)) : ?>
                    <tr style="text-align: center;"><td colspan="0"><?php echo \Text::_('com_planjeagenda_NOCONTACTS'); ?></td></tr>
                <?php else :?>
