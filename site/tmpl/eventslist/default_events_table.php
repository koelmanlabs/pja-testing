...
<?php foreach ($this->rows as $row):
    ...
    $eventUrl = KoelmanLabs\Component\Planjeagenda\Site\Helper\RouteHelper::getVenueRoute(
    $row->id . ':' . $row->alias,
    $row->catid ?? 0,
    $row->language ?? null
    );
    ...
    <div class="pja-event-card<?php echo $isFeatured ? ' featured' : ''; ?>"
        ...
        <div class="pja-event-body">
            <div class="pja-event-title" itemprop="name">
                ...
                <?php endif; ?>
                <?php echo OutputHelper::publishstateicon($row); ?>
                <?php if ($showiconsineventtitle): echo OutputHelper::recurrenceicon($row); endif; ?>
            </div>
            ...
            <div class="pja-event-meta">
                ...
                <span>
                    ...
                    <?php if ($this->jemsettings->showlinkvenue == 1 && !empty($row->venueslug)): ?>
                        <a href="<?php echo Route::_(RouteHelper::getVenueRoute($row->venueslug)); ?>"
                ...
                <span class="pja-cat-inline-mobile" style="display:none;">
                    <?php
                    $cats = OutputHelper::getCategoryList($row->categories, $this->jemsettings->catlinklist);
                    foreach ($cats as $cat):
                    ?>
