...
if ($this->showRegForm && empty($this->print)) :
    ...
    if (($this->item->maxplaces > 0) && (($this->item->booked + $this->item->reservedplaces) >= $this->item->maxplaces) && !$this->item->waitinglist && empty($this->registration->status)) :
        ?>
        <?php echo \Text::_( 'com_planjeagenda_EVENT_FULL_NOTICE' ); ?>
    ...
    <?php else :
        ...
        <form id="JEM" action="<?php echo \Route::_('index.php?option=com_planjeagenda&view=event&id=' . (int)$this->item->id); ?>"  name="adminForm" id="adminForm" method="post">
            <div>
                <?php
                if ($this->isregistered === false) {
                    if ($this->item->requestanswer) {
                        echo \Text::_('com_planjeagenda_SEND_UNREGISTRATION');
                    }
                    if ($this->item->registra == 3) {
                        echo \Text::_('com_planjeagenda_NOT_INVITED');
                    } else {
                        echo \Text::_('com_planjeagenda_YOU_ARE_UNREGISTERED');
                    }
                } else {
                    switch ($this->isregistered) :
                        case -1:
                            //You are NOT attending
                            echo \Text::_('com_planjeagenda_YOU_ARE_NOT_ATTENDING');
                            break;
                        case  0:
                            //You're invited
                            echo \Text::_('com_planjeagenda_YOU_ARE_INVITED');
                            break;
                        case  1:
                            //You're attending
                            if ($this->allowAnnulation) {
                                echo \Text::_('com_planjeagenda_YOU_ARE_ATTENDING');
                            } else {
                                echo substr(\Text::_('com_planjeagenda_YOU_ARE_ATTENDING'), 0,strpos(\Text::_('com_planjeagenda_YOU_ARE_ATTENDING'), "<br>"));
                            }
                            break;
                        case  2:
                            //You're on Waitinglist
                            echo \Text::_('com_planjeagenda_YOU_ARE_ON_WAITINGLIST');
                            break;
                        default:
                            //You didn't answer!
                            echo \Text::_('com_planjeagenda_YOU_ARE_UNREGISTERED');
                ...
                if ($this->item->seriesbooking) {
                    ...
                    if ($events) {
                        ...
                        echo '<div class="pt-3">' . \Text::_('com_planjeagenda_I_WILL_NOT_GO_SERIES_4') . '</div>';
                        echo '<div><table id="table-series"><thead><tr><th>' . \Text::_('com_planjeagenda_DATE') . '</th><th>' . \Text::_('com_planjeagenda_TITLE') . '</th><th>' . \Text::_('com_planjeagenda_STATUS') . '</th><th>' . \Text::_('com_planjeagenda_PLACES') . '</th><th>ID</th></tr></thead><tbody>';

                        foreach ($events as $e) {
                            if (!$e->waiting && $e->status == 1) {
                                $status = \Text::_('com_planjeagenda_ATTENDEES_ATTENDING');
                            } else if ($e->waiting == 1 && $e->status == 1) {
                                $status = \Text::_('com_planjeagenda_ATTENDEES_ON_WAITINGLIST');
                            } else if (!$e->status){
                                $status = \Text::_('com_planjeagenda_ATTENDEES_INVITED');
                            } else if ($e->status == -1) {
                                $status = \Text::_('com_planjeagenda_ATTENDEES_NOT_ATTENDING');
                            } else {
                                $status = \Text::_('com_planjeagenda_ATTENDEES_STATUS_UNKNOWN');
                            }
            ...
            <ul class="eventlist">
                <li class="klevents-event" onclick="document.getElementById('jem_register_event').click();">
                    ...
                    />
                    <i class="fa fa-check-circle-o fa-lg klevents-registerbutton" aria-hidden="true"></i>
                    <?php
                    ...
                    if ($this->item->maxplaces && (($this->item->booked + $this->item->reservedplaces) >= $this->item->maxplaces) && $placesavailableuser !==    0) {
                        if ($this->item->waitinglist) {
                            if ($placesBookedUser) {
                                ...
                                echo \Text::_('com_planjeagenda_EVENT_FULL_USER_REGISTERED_NO_WAITING_LIST');
                            } else {
                                echo \Text::_('com_planjeagenda_EVENT_FULL_REGISTER_TO_WAITING_LIST');
                            }
                        } else {
                            if ($placesavailableevent === 0) {
                                echo \Text::_('com_planjeagenda_NOT_AVAILABLE_PLACES_EVENT');
                                $placesavailableuser = 0;
                            }
                        }
                    } else {
                        ...
                        if ($this->registereduser !== null) {
                            if (!$placesBookedUser) {
                                echo \Text::_('com_planjeagenda_I_WILL_GO');
                            }
                        } else {
                            echo \Text::_('com_planjeagenda_I_WILL_GO');
                            if(!$this->allowRegistration){
                                echo '<span class="badge bg-warning text-light" role="alert">' . \Text::_('com_planjeagenda_EVENT_REGISTRATION_CLOSED') . '</span>';
                            }
                        }
                    }
                    ...
                    if ($placesavailableuser === 0 || $this->registration === false) {
                        echo '<span class="badge bg-warning text-light" role="alert">' . \Text::_('com_planjeagenda_NOT_AVAILABLE_PLACES_USER') . '</span>';
                    } else {
                        // Booking places
                        if ($this->item->maxbookeduser > 1) {
                            echo ' ' . \Text::_('com_planjeagenda_I_WILL_GO_2');
                            ...
                            if ($this->registereduser != null) {
                                ...
                                if ($placesBookedUser && $statusRegistrationUser == 1) {
                                    echo ' ' . \Text::_('com_planjeagenda_I_WILL_GO_3');
                                } else {
                                    //Place
                                    echo ' ' . \Text::_('com_planjeagenda_PLACES_REG') . '.';
                                }
                            } else {
                                //Place
                        ...
                        if ($this->item->recurrence_type){
                            echo '<div class="p-3" >' . \Text::_('com_planjeagenda_I_WILL_GO_SERIES_1') . '</div>';
                            if ($this->item->seriesbooking) {
                                // If event has 'seriesbooking' active and $checkseries is true then get all recurrence events of series from now (register or unregister)
                                ...
                                if($events) {
                                    // Shown the active series event list
                                    echo '<div class="px-3">' . \Text::_('com_planjeagenda_I_WILL_GO_SERIES_4') . '</div>';
                                    echo '<div class="px-3"><table id="table-series"><thead><tr><th>' . \Text::_('com_planjeagenda_DATE') . '</th><th>' . \Text::_('com_planjeagenda_TITLE') . '</th>' . ($this->registereduser? '<th>' . \Text::_('com_planjeagenda_STATUS') . '</th><th>' . \Text::_('com_planjeagenda_PLACES') . '</th>':'') . '<th>ID</th></tr></thead><tbody>';
                                    ...
                                    if ($this->item->singlebooking) {
                                        echo '<div class="px-3 pt-3"> <input id = "jem_unregister_event_series" type = "checkbox"  name = "reg_check_series"> ' . \Text::_('com_planjeagenda_I_WILL_GO_SERIES_2') . '</input ></div>';
                                    } else {
                                        echo '<div class="px-3 pt-3">' . \Text::_('com_planjeagenda_I_WILL_GO_SERIES_3') . '</div>';
                                    }
                                }
                            }
                        }
                ...
                <?php if ($this->item->requestanswer || $placesRegisteredUser || $waitingPlacesUser || $this->allowAnnulation) {?>
                    <li class="klevents-event" onclick="document.getElementById('jem_unregister_event').click();">
                        ...
                        <?php if ($this->allowAnnulation || ($this->isregistered != 1) || $waitingPlacesUser) : ?>
                            ...
                            />
                            <i class="fa fa-times-circle-o fa-lg klevents-unregisterbutton" aria-hidden="true"></i>
                            <?php
                            //Option: I don't attend
                            echo ' ' . \Text::_('com_planjeagenda_I_WILL_NOT_GO');
                            if ($this->registereduser !== null || $waitingPlacesUser) {
                                if ($placesRegisteredUser || $waitingPlacesUser) {
                                    if ($statusRegistrationUser == 1) {
                                        // Booked places
                                        $cancelplaces = ($placesRegisteredUser - 1 > 1 ? \Text::_('com_planjeagenda_BOOKED_PLACES') : \Text::_('com_planjeagenda_BOOKED_PLACE'));
                                    } else if ($statusRegistrationUser == -1) {
                                        $cancelplaces = '';
                                        //Booked places for invited users
                                    } else if ($statusRegistrationUser == 0) {
                                        $cancelplaces = ($placesRegisteredUser - 1 > 1 ? \Text::_('com_planjeagenda_INVITED_PLACES') : \Text::_('com_planjeagenda_INVITED_PLACE'));
                                        //Booked places for waiting users
                                    } else if ($statusRegistrationUser == 2) {
                                        $cancelplaces = ($waitingPlacesUser - 1 > 1 ? \Text::_('com_planjeagenda_WAITING_PLACES') : \Text::_('com_planjeagenda_WAITING_PLACE'));
                                    }

                                    //Canceling...
                                    echo ' ' . \Text::_('com_planjeagenda_I_WILL_NOT_GO_2');
                                ...
                                if ($this->item->recurrence_type) {
                                    echo '<div class="pt-3 pl-3">' . \Text::_('com_planjeagenda_I_WILL_NOT_GO_SERIES_1') . '</div>';
                                    if ($this->item->seriesbooking) {
                                        if($events) {
                                            if ($this->item->singlebooking) {
                                                echo '<div class="px-3 pt-3"> <input id = "jem_unregister_event_series" type = "checkbox"  name = "reg_check_series"> ' . \Text::_('com_planjeagenda_I_WILL_NOT_GO_SERIES_2') . '</input ></div>';
                                            } else {
                                                echo '<div class="px-3">' . \Text::_('com_planjeagenda_I_WILL_NOT_GO_SERIES_3') . '</div>';
                                            }
                                        }else{
                                            echo '<div class="px-3">' . \Text::_('com_planjeagenda_I_WILL_NOT_GO_SERIES_5') . '</div>';
                                        }
                                    }
                                }
                            } else {
                                //...booked places
                                $cancelplaces = \Text::_('com_planjeagenda_I_WILL_NOT_GO_3');
                            }
                            ?>
                        <?php else :
                            ...
                            <i class="fa fa-times-circle-o fa-lg klevents-unregisterbutton" aria-hidden="true"></i>
                            <?php echo ' ' . \Text::_('com_planjeagenda_NOT_ALLOWED_TO_ANNULATE'); ?>
                        <?php endif; ?>
                    </li>
                <?php }
                ...
                <?php if (!empty($this->jemsettings->regallowcomments)) { ?>
                    <li class="klevents-event klevents-nopointer klevents-nohover">
                        <p><?php echo \Text::_('com_planjeagenda_OPTIONAL_COMMENT') . ':'; ?></p>
                        <div class="klevents-regcomment">
            ...
            <input class="btn btn-sm btn-primary" type="submit" id="jem_send_attend" name="jem_send_attend"
                <?php echo ($disabledOptions? 'disabled="disabled"':'');?>
                   value="<?php echo ($placesRegisteredUser ? \Text::_('com_planjeagenda_SEND_REGISTER') : \Text::_('com_planjeagenda_REGISTER')); ?>"  />
