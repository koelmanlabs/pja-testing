 ...
 *
 */
class PlanjeagendaModelAttendee extends BaseDatabaseModel
{
    /**
     * Attendee id
     *
    ...
    public function store($data)
    {
        ...
        if (!$row->id) {
            ...
            $query = ' SELECT e.maxplaces, e.waitinglist, COUNT(r.id) as booked '
                   ...
                   . ' INNER JOIN #__pja_register AS r ON r.event = e.id '
            ...
            if ($details->maxplaces > 0) // there is a max
            {
                ...
                if ($details->booked >= $details->maxplaces)
                {
                    if (!$details->waitinglist) {
                        \Joomla\CMS\Factory::getApplication()->enqueueMessage(Text::_('com_planjeagenda_ERROR_REGISTER_EVENT_IS_FULL'), 'warning');
                        return false;
                    }
                    $row->waiting = 1;
                }
