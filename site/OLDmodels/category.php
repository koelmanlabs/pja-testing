 ...
 */
class PlanjeagendaModelCategory extends PlanjeagendaModelEventslist
{
    protected $_id            = null;
    ...
    protected function populateState($ordering = null, $direction = null)
    {
        ...
        if ($app->input->getInt('limitstart', null) === null) {
            $app->setUserState('com_planjeagenda.category.'.$itemid.'.limitstart', 0);
        }

        if (empty($format) || ($format == 'html')) {
            $limit = $app->getUserStateFromRequest('com_planjeagenda.category.'.$itemid.'.limit', 'limit', $jemsettings->display_num, 'int');
            $this->setState('list.limit', $limit);

            $limitstart = $app->getUserStateFromRequest('com_planjeagenda.category.'.$itemid.'.limitstart', 'limitstart', 0, 'int');
        ...
        $month = $app->getUserStateFromRequest('com_planjeagenda.category.'.$itemid.'.filter_month', 'filter_month', '', 'string');
        $this->setState('filter.filter_month', $month);

        $filtertype = $app->getUserStateFromRequest('com_planjeagenda.category.'.$itemid.'.filter_type', 'filter_type', 0, 'int');
        ...
        $filter_order_Dir = $app->getUserStateFromRequest('com_planjeagenda.category.'.$itemid.'.filter_order_Dir', 'filter_order_Dir', $filter_order_DirDefault, 'word');
    ...
    public function getCategory()
    {
        if (!is_object($this->_item)) {
            ...
            $categories = new PlanjeagendaCategories($catId, $options);
            $this->_item = $categories->get($catId);

            // Compute selected asset permissions.
            if (is_object($this->_item)) { // a PlanjeagendaCategoryNode object
                $user = Factory::getApplication()->getIdentity();
                $asset    = 'com_planjeagenda.category.'.$this->_item->id;
