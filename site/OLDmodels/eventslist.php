...
// ensure PlanjeagendaFactory is loaded (because model is used by modules too)
require_once(JPATH_SITE.'/components/com_planjeagenda/factory.php');

/**
 * Model-Eventslist
 **/
class PlanjeagendaModelEventslist extends ListModel
{
    /**
     * Constructor.
     */
    ...
    protected function populateState($ordering = null, $direction = null)
    {
        ...
        if (empty($format) || ($format == 'html')) {
            ...
            if ($app->input->get('limitstart', null, 'int') === null) {
                $app->setUserState('com_planjeagenda.eventslist.' . $itemid . '.limitstart', 0);
            }

            $limit      = $app->getUserStateFromRequest('com_planjeagenda.eventslist.' . $itemid . '.limit', 'limit', $jemsettings->display_num, 'int');
            $this->setState('list.limit', $limit);
            $limitstart = $app->getUserStateFromRequest('com_planjeagenda.eventslist.' . $itemid . '.limitstart', 'limitstart', 0, 'int');
        ...
        // Region/province filter — set by the regio map module or filter bar
        ...
        // Venue text and city text filters — from expanded filter panel
        ...
        // Category filter — from filter bar dropdown
        ...
        // Date range filters — from filter bar date pickers
                ...
                if($published<2) {
                    if (empty($jubilee_filter) || !$jubilee_show_past) {
                        if ($cal_month) {
                            ...
                            // Range comparison lets MySQL use idx_dates — DATEDIFF() would disable it.
