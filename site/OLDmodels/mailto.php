 ...
 * @since  3.8.9
 */
class PlanjeagendaModelMailto extends FormModel
{
    /**
    ...
    public function getForm($data = array(), $loadData = true)
    {
        ...
        // Get the form.
        $form = $this->loadForm('com_planjeagenda.mailto', 'mailto', array('load_data' => $loadData));
    ...
    protected function loadFormData()
    {
        ...
        $data = $app->getUserState('klevents.mailto.form.data', array());

        $data['link'] = urldecode($app->input->get('link', '', 'BASE64'));

        if ($data['link'] == '')
        {
            // JError::raiseError(403, Text::_('com_planjeagenda_MAILTO_LINK_IS_MISSING'));
            Factory::getApplication()->enqueueMessage(Text::_('com_planjeagenda_MAILTO_LINK_IS_MISSING'), 'error');
        ...
        $app->setUserState('klevents.mailto.form.data', $data);

        $this->preprocessData('com_planjeagenda.mailto', $data);
