 ...
 * Model-Day
 */
class PlanjeagendaModelDay extends PlanjeagendaModelEventslist
{
    ...
    public function setDate($date)
    {
        ...
        # a valid date has 8 characters (ymd)
            ...
            if (checkdate($month, $day, $year)) {
                ...
                $date = date('Ymd');
                Factory::getApplication()->enqueueMessage(Text::_('com_planjeagenda_INVALID_DATE_REQUESTED_USING_CURRENT'), 'notice');
            }
        } else {
            //date isn't valid raise notice and use current date
            $date = date('Ymd');
    ...
    protected function populateState($ordering = null, $direction = null)
    {
        ...
        $locid = $app->getUserState('com_planjeagenda.venuecal.locid'.$item);
        if ($locid) {
        ...
        $cal_category_catid = $app->getUserState('com_planjeagenda.categorycal.catid'.$item);
...
################################
        ...
        if ($top_category > 0) { // accept 'root'
            $children = PlanjeagendaCategories::getChilds($top_category);
            if (count($children)) {
                $where = 'c.id IN ('. implode(',', $children) .')';
        ...
        if ($app->input->getInt('limitstart', null) === null) {
            $app->setUserState('com_planjeagenda.day.'.$itemid.'.limitstart', 0);
        }

        $limit = $app->getUserStateFromRequest('com_planjeagenda.day.'.$itemid.'.limit', 'limit', $jemsettings->display_num, 'int');
        $this->setState('list.limit', $limit);

        $limitstart = $app->getUserStateFromRequest('com_planjeagenda.day.'.$itemid.'.limitstart', 'limitstart', 0, 'int');
        $limitstart = $limit ? (int)(floor($limitstart / $limit) * $limit) : 0;
        $this->setState('list.start', $limitstart);

        # Search
        $search = $app->getUserStateFromRequest('com_planjeagenda.day.'.$itemid.'.filter_search', 'filter_search', '', 'string');
        $this->setState('filter.filter_search', $search);

        # FilterType
        $filtertype = $app->getUserStateFromRequest('com_planjeagenda.day.'.$itemid.'.filter_type', 'filter_type', 0, 'int');
        $this->setState('filter.filter_type', $filtertype);

        # filter_order
        $orderCol = $app->getUserStateFromRequest('com_planjeagenda.day.'.$itemid.'.filter_order', 'filter_order', 'a.dates', 'cmd');
        $this->setState('filter.filter_ordering', $orderCol);

        # filter_direction
        $listOrder = $app->getUserStateFromRequest('com_planjeagenda.day.'.$itemid.'.filter_order_Dir', 'filter_order_Dir', 'ASC', 'word');
