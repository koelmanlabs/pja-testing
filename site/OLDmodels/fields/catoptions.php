...
if (!class_exists('PlanjeagendaHelper')) { require_once __DIR__ . '/../../helpers/helper.php'; }
...
class JFormFieldCatOptions extends ListField
{
    ...
    public function getInput()
    {
        ...
        if(empty($currentid)) {
            ...
            // Single-category: get catid from pja_events directly
            $query2 = $db->getQuery(true)->select('catid')->from('#__pja_events')->where('id = ' . (int)$currentid);
            $db->setQuery($query2);
            $catid = (int)$db->loadResult();
            $selectedcats = $catid ? [$catid] : [];
            // (query below not used for selection anymore)
            $query->where('1=0'); // disabled
            $db->setQuery($query);
        ...
        if (count($mitems) > count($list)) {
            foreach ($children as $k => $v) {
                if (($k > 1) && !array_key_exists($k, $list)) {
                    $list = PlanjeagendaCategories::treerecurse($k, '?&nbsp;', $list, $children, 999, 0, 0);
                }
            }
        }
