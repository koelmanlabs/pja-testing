 ...
 *
*/
class PlanjeagendaModelMyevents extends BaseDatabaseModel
{
    /**
     * Events data array
     *
    ...
    public function __construct()
    {
        ...
        if ($app->input->getInt('limitstart', null) === null) {
            $app->setUserState('com_planjeagenda.myevents.limitstart', 0);
        }

        $limit      = $app->getUserStateFromRequest('com_planjeagenda.myevents.limit', 'limit', $jemsettings->display_num, 'int');
        $limitstart = $app->getUserStateFromRequest('com_planjeagenda.myevents.limitstart', 'limitstart', 0, 'int');
    ...
    public function getEvents()
    {
        ...
        if ($this->_events) {
            ...
            foreach ($this->_events as $i => $item) {
                ...
                if (empty($item->categories)) {
                    ...
                    if (empty($item->params)) {
                        ...
                        $item->params = clone PlanjeagendaHelper::globalattribs();
                        $item->params->merge($registry);
                    }
                    # edit state access permissions.
                    $item->params->set('access-change', $user->authorise('core.edit.state', 'com_planjeagenda'));
            ...
            PlanjeagendaHelper::getAttendeesNumbers($this->_events); // does directly edit events
        }
    ...
    protected function _buildOrderBy()
    {
        ...
        $filter_order_Dir  = $app->getUserStateFromRequest('com_planjeagenda.myevents.filter_order_Dir', 'filter_order_Dir', 'ASC', 'word');
    ...
    public function getCategories($id)
    {
        ...
        $query = 'SELECT DISTINCT c.id, c.catname, c.access, c.checked_out AS cchecked_out, c.groupid,'
               ...
               . ' FROM #__pja_events AS ae2'
               . ' LEFT JOIN #__pja_categories AS c ON c.id = ae2.catid'
               . ' WHERE ae2.id = '.(int)$id
               . ' AND c.published = 1'
