...
class PlanjeagendaHtml
{
    ...
    static public function featured($i, $value = 0, $canChange = true)
    {
        ...
        $html = HTMLHelper::_('jemhtml.icon', 'com_planjeagenda/'.$state[0], 'fa fa-fw fa-lg '.$state[1].' klevents-featured-'.$state[1], $state[3], null, $no_iconfont);
        if ($canChange) {
...

    {
        ...
        $state = ArrayHelper::getValue($states, (int)$value, $states[1]);
        $no_iconfont = (bool)Factory::getApplication()->isClient('administrator'); // requires font and css loaded which isn't yet on backend
        $html = HTMLHelper::_('jemhtml.icon', 'com_planjeagenda/'.$state[0], 'fa fa-fw fa-lg '.$state[1].' klevents-attendance-status-'.$state[1], $state[3], null, $no_iconfont);
        if ($canChange) {
            $html = '<a href="#" onclick="return Joomla.listItemTask(\'cb' . $i . '\',\'' . $state[2] . '\')" title="' . Text::_($state[4]) . '">' . $html . '</a>';
        }
...

    {
        ...
        if ($print) {
            $html = jemhtml::icon('com_planjeagenda/'.$state[0], 'fa fa-fw fa-lg '.$state[1].' klevents-attendance-status-'.$state[1], $state[3], 'class="icon-inline-left"', $backend);
            $html .= Text::_($state[5]);
        } elseif ($canChange && !empty($state[2])) {
            $html = jemhtml::icon('com_planjeagenda/'.$state[0], 'fa fa-fw fa-lg '.$state[1].' klevents-attendance-status-'.$state[1], $state[3], null, $backend);
            if ($backend) {
            ...
            $html = jemhtml::icon('com_planjeagenda/'.$state[0], 'fa fa-fw fa-lg '.$state[1].' klevents-attendance-status-'.$state[1], $state[3], $attr, $backend);
        }
