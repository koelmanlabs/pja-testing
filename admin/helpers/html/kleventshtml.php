...
class PlanjeagendaHtml
{
    ...
    static public function featured($i, $value = 0, $canChange = true)
    {
        ...
        $states = array(
                0 => array(
                        ...
                        'events.featured',
                        'com_planjeagenda_EVENTS_UNFEATURED',
                        'com_planjeagenda_EVENTS_TOGGLE_TO_FEATURE'
                ),
                1 => array(
                        'featured.webp',
                        'fa-star', //'fa-circle',
                        'events.unfeatured',
                        'com_planjeagenda_EVENTS_FEATURED',
                        'com_planjeagenda_EVENTS_TOGGLE_TO_UNFEATURE'
                )
        );
        ...
        $html = HTMLHelper::_('kleventshtml.icon', 'com_planjeagenda/'.$state[0], 'fa fa-fw fa-lg '.$state[1].' klevents-featured-'.$state[1], $state[3], null, $no_iconfont);
        if ($canChange) {
...

    {
        if (class_exists('PlanjeagendaHelper')) {
            PlanjeagendaHelper::addLogEntry('Use of this function is deprecated. Use PlanjeagendaHelper::toggleAttendanceStatus() instead.', __METHOD__, Log::WARNING);
        }
        ...
        $state = ArrayHelper::getValue($states, (int)$value, $states[1]);
        $no_iconfont = (bool)Factory::getApplication()->isClient('administrator'); // requires font and css loaded which isn't yet on backend
        $html = HTMLHelper::_('kleventshtml.icon', 'com_planjeagenda/'.$state[0], 'fa fa-fw fa-lg '.$state[1].' klevents-attendance-status-'.$state[1], $state[3], null, $no_iconfont);
        if ($canChange) {
            $html = '<a href="#" onclick="return Joomla.listItemTask(\'cb' . $i . '\',\'' . $state[2] . '\')" title="' . Text::_($state[4]) . '">' . $html . '</a>';
        }
...

    {
        ...
        if ($print) {
            $html = PlanjeagendaHtml::icon('com_planjeagenda/'.$state[0], 'fa fa-fw fa-lg '.$state[1].' klevents-attendance-status-'.$state[1], $state[3], 'class="icon-inline-left"', $backend);
            $html .= Text::_($state[5]);
        } elseif ($canChange && !empty($state[2])) {
            $html = PlanjeagendaHtml::icon('com_planjeagenda/'.$state[0], 'fa fa-fw fa-lg '.$state[1].' klevents-attendance-status-'.$state[1], $state[3], null, $backend);
            if ($backend) {
            ...
            $html = PlanjeagendaHtml::icon('com_planjeagenda/'.$state[0], 'fa fa-fw fa-lg '.$state[1].' klevents-attendance-status-'.$state[1], $state[3], $attr, $backend);
        }
     ...
     * Call HTMLHelper::_('kleventshtml.icon', $image, $icon, $alt, $attribs, $no_iconfont, $relative)
     *
    ...
    {
        $useiconfont = !$no_iconfont && (PlanjeagendaHelper::config()->useiconfont == 1);
...
// Backwards compatibility alias - HTMLHelper::_('kleventshtml.xxx') still works
if (!class_exists('JemHtml')) {
    class_alias('PlanjeagendaHtml', 'JemHtml');
}
// Also register as 'jemhtml' for HTMLHelper service locator
if (!class_exists('jemhtml')) {
    class_alias('PlanjeagendaHtml', 'jemhtml');
}
