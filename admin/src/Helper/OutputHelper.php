...
use KoelmanLabs\Component\Planjeagenda\Site\helpers\Icon;
...
class OutputHelper
{
    ...
    static public function footer()
    {
        ...
        if ($app->input->get('print','','int')) {
            ...
            echo 'Powered by <a href="https://www.koelmanlabs.nl" target="_blank" title="Joomla Event Manager">JEM</a>';
        }
    }
...
static public function lightbox() {
    ...
    if ($settings->lightbox == 1) {
        ...
        $document->addStyleSheet(Uri::base() .'media/com_planjeagenda/css/lightbox.min.css');
        $document->addScript(Uri::base() . 'media/com_planjeagenda/js/lightbox.min.js');
    ...
    static public function createButtonBar($view, $permissions, $params)
    {
        ...
        if (!$archive) {
            if (in_array('addEvent', $btns_show) || (!in_array('addEvent', $btns_hide) && in_array($view, array('calendar', 'categories', 'category', 'day', 'event', 'eventslist', 'myevents', 'myvenues', 'venue', 'venues')))) {
                $buttons[$idx][] = OutputHelper::submitbutton(!empty($canAddEvent), null);
            }
            if (in_array('addVenue', $btns_show) || (!in_array('addVenue', $btns_hide) && in_array($view, array('calendar', 'categories', 'category', 'day', 'event', 'eventslist', 'myevents', 'myvenues', 'venue', 'venues', 'venueslist')))) {
                $buttons[$idx][] = OutputHelper::addvenuebutton(!empty($canAddVenue), null, null);
            }
            if (in_array('addUsers', $btns_show) || (!in_array('addUsers', $btns_hide) && in_array($view, array('attendees')))) {
                $buttons[$idx][] = OutputHelper::addusersbutton(!empty($canAddUsers), $id);
            }
        }
        ...
        if (in_array('archive', $btns_show) || (!in_array('archive', $btns_hide) && in_array($view, array('categories', 'category', 'eventslist', 'myattendances', 'myevents', 'venue')))) {
            $buttons[$idx][] = OutputHelper::archivebutton($archive_link, $task , $id); // task: archive, id: for '&id='
        }
        if (in_array('mail', $btns_show) || (!in_array('mail', $btns_hide) && in_array($view, array('category', 'event', 'venue', 'venueslist')))) {
            $buttons[$idx][] = OutputHelper::mailbutton($slug, $view, null); // slug: for '&id='
        }
    ...
    static public function submitbutton($dellink, $params)
    {
        if ($dellink)
        {
            ...
            $settings2 = PlanjeagendaHelper::config();
            $uri       = Uri::getInstance();
            $app = Factory::getApplication();
 ...
 // @todo: FIX
               // $image = PlanjeagendaHtml::icon( 'com_planjeagenda/submitevent.webp', 'fa fa-fw fa-lg fa-calendar-plus klevents-submitbutton', Text::_('com_planjeagenda_DELIVER_NEW_EVENT'), NULL, !$app->isClient('site'));
            } else {
                $image = Text::_('com_planjeagenda_DELIVER_NEW_EVENT');
            }

            $url = 'index.php?option=com_planjeagenda&task=event.add&return='.base64_encode($uri).'&a_id=0';
            $overlib = Text::_('com_planjeagenda_SUBMIT_EVENT_DESC');
            $output = HTMLHelper::_('link', Route::_($url), $image, self::tooltip(Text::_('com_planjeagenda_DELIVER_NEW_EVENT'), $overlib, '', 'bottom'));
    ...
    static public function addvenuebutton($addvenuelink, $params, $settings2)
    {
        if ($addvenuelink) {
            ...
            if ($settings->get('global_show_icons',1)) {
// @todo FIX    $image = PlanjeagendaHtml::icon( 'com_planjeagenda/addvenue.webp', 'fa fa-fw fa-lg fa-plus-square klevents-addvenuebutton', Text::_('com_planjeagenda_DELIVER_NEW_VENUE'), NULL, !$app->isClient('site'));
            } else {
                $image = Text::_('com_planjeagenda_DELIVER_NEW_VENUE');
            }

            $url = 'index.php?option=com_planjeagenda&task=venue.add&return='.base64_encode($uri).'&a_id=0';
            $overlib = Text::_('com_planjeagenda_DELIVER_NEW_VENUE_DESC');
            $output = HTMLHelper::_('link', Route::_($url), $image, self::tooltip(Text::_('com_planjeagenda_DELIVER_NEW_VENUE'), $overlib, '', 'bottom'));
    ...
    static public function addusersbutton($adduserslink, $eventid)
    {
        if ($adduserslink) {
            ...
            if ($settings->get('global_show_icons',1)) {
                $image = PlanjeagendaHtml::icon( 'com_planjeagenda/icon-16-new.webp', 'fa fa-fw fa-lg fa-user-plus klevents-addusersbutton', Text::_('com_planjeagenda_ADD_USER_REGISTRATIONS'), NULL, !$app->isClient('site'));
            } else {
                $image = Text::_('com_planjeagenda_ADD_USER_REGISTRATIONS');
            }

            $url = 'index.php?option=com_planjeagenda&view=attendees&layout=addusers&tmpl=component&return='.base64_encode($uri).'&id='.$eventid.'&'.Session::getFormToken().'=1';
            $overlib = Text::_('com_planjeagenda_ADD_USER_REGISTRATIONS_DESC');
            // $output = HTMLHelper::_('link', Route::_($url), $image, self::tooltip(Text::_('com_planjeagenda_ADD_USER_REGISTRATIONS'), $overlib, 'flyermodal', 'bottom').' rel="{handler: \'iframe\', size: {x:800, y:450}}"');
            ...
            $output.='<a href="javascript:void(0);" data-bs-toggle="modal" data-bs-target="#adduser-modal" ' .  self::tooltip(Text::_('com_planjeagenda_ADD_USER_REGISTRATIONS'), $overlib, 'flyermodal', 'bottom'). '>' . $image . '</a>';
    ...
    static public function removebutton($text, $attributes = array())
    {
        ...
        $output = PlanjeagendaHtml::icon( 'com_planjeagenda/publish_r.webp', 'fa fa-lg fa-times-circle klevents-removebutton', $text, $attributes, !$app->isClient('site'));
    ...
    static public function prepareAddEventButton($urlparams = '')
    {
        ...
        $app   = Factory::getApplication();
        $image = PlanjeagendaHtml::icon( 'com_planjeagenda/submitevent.webp', 'fa fa-fw fa-lg fa-calendar-plus klevents-submitbutton', Text::_('com_planjeagenda_DELIVER_NEW_EVENT'), NULL, !$app->isClient('site'));
        $url   = 'index.php?option=com_planjeagenda&task=event.add&a_id=0&date={date}&return='.base64_encode($uri);
        if (!empty($urlparams) && preg_match('/^[a-z]+=\w+$/i', $urlparams)) {
            $url .= '&'.$urlparams;
        }
        $html  = '<div class="inline-button-right">';
        $html .= HTMLHelper::_('link', Route::_($url), $image, self::tooltip(Text::_('com_planjeagenda_DELIVER_NEW_EVENT'), Text::_('com_planjeagenda_SUBMIT_EVENT_DESC'), '', 'bottom'));
        $html .= '</div>';
    ...
    static public function archivebutton($archive_link, $task = NULL, $id = NULL)
    {
        ...
        if ($settings->get('global_show_archive_icon',1)) {
            ...
            if ($task == 'archive') {
                if ($settings->get('global_show_icons',1)) {
                    $image = self::makeicon( 'com_planjeagenda/el.webp', 'fa fa-fw fa-lg fa-calendar klevents-archivebutton-return', Text::_('com_planjeagenda_SHOW_EVENTS'), NULL, !$app->isClient('site'));
                } else {
                    $image = Text::_('com_planjeagenda_SHOW_EVENTS');
                }

                // TODO: Title and overlib just fit to events view
                $overlib = Text::_('com_planjeagenda_SHOW_EVENTS_DESC');
                $title = Text::_('com_planjeagenda_SHOW_EVENTS');
                ...
                if ($settings->get('global_show_icons',1)) {
                       $image = self::makeicon( 'com_planjeagenda/archive_front.webp', 'fa fa-fw fa-lg fa-archive klevents-archivebutton', Text::_('com_planjeagenda_SHOW_ARCHIVE'), NULL, !$app->isClient('site'));
                } else {
                    $image = Text::_('com_planjeagenda_SHOW_ARCHIVE');
                }

                $overlib = Text::_('com_planjeagenda_SHOW_ARCHIVE_DESC');
                $title = Text::_('com_planjeagenda_SHOW_ARCHIVE');
    ...
    static public function editbutton($item, $params, $attribs, $allowedtoedit, $view)
    {
        if ($allowedtoedit) {
            ...
            if (version_compare(JVERSION, '5.0.0', '>=')) {
                ...
                $iconEditEventRoot='fa-sharp fa-solid fa-pen-to-square klevents-editbutton';
            } elseif (version_compare(JVERSION, '4.0.0', '>=')) {
                // Joomla 4 with Font Awesome 5
                $iconEditEventRoot='fa fa-fw fa-edit klevents-editbutton';
            }
            ...
            switch ($view)
            {
                case 'editevent':
                    if (property_exists($item, 'checked_out') && property_exists($item, 'checked_out_time') && $item->checked_out > 0 && $item->checked_out != $userId) {
                        ...
                        return '<span ' . self::tooltip(Text::_('JLIB_HTML_CHECKED_OUT'), htmlspecialchars(Text::sprintf('com_planjeagenda_GLOBAL_CHECKED_OUT_BY', $checkoutUser->name) . ' <br> ' . $date, ENT_COMPAT, 'UTF-8')) . '>' . $button . '</span>';
                    }

                    if ($showIcon) {
                        if($item->recurrence_type && !$item->recurrence_first_id){
                            $image = PlanjeagendaHtml::icon('com_planjeagenda/calendar_edit_root.webp', $iconEditEventRoot, Text::_('com_planjeagenda_EDIT_EVENT_ROOT'), NULL, !$app->isClient('site'));
                            $overlib = Text::_('com_planjeagenda_EDIT_EVENT_ROOT_DESC');
                            $text = Text::_('com_planjeagenda_EDIT_EVENT_ROOT');
                        }else {
                            $image = PlanjeagendaHtml::icon('com_planjeagenda/calendar_edit.webp', 'fa fa-fw fa-pen-square klevents-editbutton', Text::_('com_planjeagenda_EDIT_EVENT'), NULL, !$app->isClient('site'));
                            $overlib = Text::_('com_planjeagenda_EDIT_EVENT_DESC');
                            $text = Text::_('com_planjeagenda_EDIT_EVENT');
                        }
                    } else {
                        $image = Text::_('com_planjeagenda_EDIT_EVENT');
                        $overlib = Text::_('com_planjeagenda_EDIT_EVENT_DESC');
                        $text = Text::_('com_planjeagenda_EDIT_EVENT');
                    }
                    $id = isset($item->did) ? $item->did : $item->id;
                    $url = 'index.php?option=com_planjeagenda&task=event.edit&a_id='.$id.'&return='.base64_encode($uri);
                    break;
                ...
                case 'editvenue':
                    ...
                    if ($showIcon) {
                        $image = PlanjeagendaHtml::icon( 'com_planjeagenda/calendar_edit.webp', 'fa fa-fw fa-pen-square klevents-editbutton', Text::_('com_planjeagenda_EDIT_VENUE'), NULL, !$app->isClient('site'));
                    } else {
                        $image = Text::_('com_planjeagenda_EDIT_VENUE');
                    }
                    $id = $item->locid;
                    $overlib = Text::_('com_planjeagenda_EDIT_VENUE_DESC');
                    $text = Text::_('com_planjeagenda_EDIT_VENUE');
                    $url = 'index.php?option=com_planjeagenda&task=venue.edit&a_id='.$id.'&return='.base64_encode($uri);
                    break;
    ...
    static public function copybutton($item, $params, $attribs, $allowedtoadd, $view)
    {
        if ($allowedtoadd) {
            ...
            switch ($view)
            {
                case 'editevent':
                    if ($showIcon) {
                        $image = PlanjeagendaHtml::icon( 'com_planjeagenda/calendar_copy.webp', 'fas fa-fw fa-copy klevents-copybutton', Text::_('com_planjeagenda_COPY_EVENT'), NULL, !$app->isClient('site'));
                    } else {
                        $image = Text::_('com_planjeagenda_COPY_EVENT');
                    }
                    $id = isset($item->did) ? $item->did : $item->id;
                    $overlib = Text::_('com_planjeagenda_COPY_EVENT_DESC');
                    $text = Text::_('com_planjeagenda_COPY_EVENT');
                    $url = 'index.php?option=com_planjeagenda&task=event.copy&a_id='.$id.'&return='.base64_encode($uri);
                    break;

                case 'editvenue':
                    if ($showIcon) {
                        $image = PlanjeagendaHtml::icon( 'com_planjeagenda/calendar_copy.webp', 'fas fa-fw fa-copy klevents-copybutton', Text::_('com_planjeagenda_COPY_VENUE'), NULL, !$app->isClient('site'));
                    } else {
                        $image = Text::_('com_planjeagenda_COPY_VENUE');
                    }
                    $id = $item->locid;
                    $overlib = Text::_('com_planjeagenda_COPY_VENUE_DESC');
                    $text = Text::_('com_planjeagenda_COPY_VENUE');
                    $url = 'index.php?option=com_planjeagenda&task=venue.copy&a_id='.$id.'&return='.base64_encode($uri);
                    break;
    ...
    static public function printbutton($print_link, $params)
    {
        ...
        if ($settings->get('global_show_print_icon',0)) {
            ...
            if ($settings->get('global_show_icons',1)) {
                $image = self::makeicon( 'com_planjeagenda/printButton.webp', 'fa fa-fw fa-lg fa-print klevents-printbutton', Text::_('JGLOBAL_PRINT'), NULL, !$app->isClient('site'));
            } else {
                $image = Text::_('com_planjeagenda_PRINT');
            }

            if ($app->input->get('print','','int')) {
                //button in popup
                $overlib = Text::_('com_planjeagenda_PRINT_DESC');
                $text = Text::_('com_planjeagenda_PRINT');
    ...
    static public function mailbutton($slug, $view, $params)
    {
        ...
        if ($settings->get('global_show_email_icon')) {
            ...
            $link     = $base.Route::_('index.php?option=com_planjeagenda&view='.$view.'&id='.$slug, false);

            if (!class_exists('PlanjeagendaMailtoHelper', false)) {
                require_once JPATH_SITE . '/components/com_planjeagenda/helpers/mailtohelper.php';
            }
            $url = 'index.php?option=com_planjeagenda&tmpl=component&view=mailto&link='.PlanjeagendaMailtoHelper::addLink($link);
            $status = 'width=400,height=350,menubar=yes,resizable=yes';

            if ($settings->get('global_show_icons')) {
                $image = PlanjeagendaHtml::icon( 'com_planjeagenda/emailButton.webp', 'fa fa-fw fa-lg fa-envelope klevents-mailbutton', Text::_('JGLOBAL_EMAIL'), NULL, !$app->isClient('site'));
            } else {
                $image = Text::_('com_planjeagenda_EMAIL');
            }

            $overlib = Text::_('com_planjeagenda_EMAIL_DESC');
            $text = Text::_('com_planjeagenda_EMAIL');
            $new_html = '';
...
// @todo FIX
        ...
        /*
        PlanjeagendaDebug::log('icalbutton', 
            'show_ical=' . var_export($settings->get('global_show_ical_icon', 'NOT_SET'), true) .
            ' show_icons=' . var_export($settings->get('global_show_icons', 'NOT_SET'), true) .
            ' view=' . $view
        );
        */
 ...
 /*       
        ...
        if ((int)$settings->get('global_show_ical_icon', 0) === 1) {
            if ($app->input->get('print','','int')) {
                return;
            }

            if ((int)$settings->get('global_show_icons', 0) === 1) {
                $image = self::makeicon( 'com_planjeagenda/iCal2.0.webp', 'fa fa-fw fa-lg fa-calendar-check klevents-icalbutton', Text::_('com_planjeagenda_EXPORT_ICS'), NULL, !$app->isClient('site'));
            } else {
                $image = Text::_('com_planjeagenda_EXPORT_ICS');
            }

            $overlib = Text::_('com_planjeagenda_ICAL_DESC');
            $text = Text::_('com_planjeagenda_ICAL');

            $url = 'index.php?option=com_planjeagenda&view=' . $view . '&id=' . ($slug??0) . ($task? '&task=' . $task : '') . '&format=raw&layout=ics';
    ...
    static public function publishbutton($prefix)
    {
        ...
        if (empty($prefix) || $app->input->get('print','','int')) {
            ...
            // button in view
            $image = PlanjeagendaHtml::icon( 'com_planjeagenda/publish.webp', 'fa fa-fw fa-lg fa-check-circle klevents-publishbutton', Text::_('com_planjeagenda_PUBLISH'), NULL, !$app->isClient('site'));
            $overlib = Text::_('com_planjeagenda_PUBLISH_DESC');
            $text = Text::_('com_planjeagenda_PUBLISH');
    ...
    static public function trashbutton($prefix)
    {
        ...
        if (empty($prefix) || $app->input->get('print','','int')) {
            ...
            // button in view
            $image = PlanjeagendaHtml::icon( 'com_planjeagenda/trash.webp', 'fa fa-fw fa-lg fa-trash klevents-trashbutton', Text::_('com_planjeagenda_TRASH'), NULL, !$app->isClient('site'));
            $overlib = Text::_('com_planjeagenda_TRASH_DESC');
            $text = Text::_('com_planjeagenda_TRASH');
    ...
    static public function unpublishbutton($prefix)
    {
        ...
        if (empty($prefix) || $app->input->get('print','','int')) {
            ...
            // button in view
            $image = PlanjeagendaHtml::icon( 'com_planjeagenda/unpublish.webp', 'fa fa-fw fa-lg fa-eye-slash klevents-unpublishbutton', Text::_('com_planjeagenda_UNPUBLISH'), NULL, !$app->isClient('site'));
            $overlib = Text::_('com_planjeagenda_UNPUBLISH_DESC');
            $text = Text::_('com_planjeagenda_UNPUBLISH');
    ...
    static public function exportbutton($eventid)
    {
        ...
        $image = PlanjeagendaHtml::icon( 'com_planjeagenda/export_excel.webp', 'fa fa-fw fa-lg fa-download klevents-exportbutton', Text::_('com_planjeagenda_EXPORT'), NULL, !$app->isClient('site'));
        ...
        if ($app->input->get('print','','int')) {
            ...
            //button in view
            $overlib = Text::_('com_planjeagenda_EXPORT_DESC');
            $text = Text::_('com_planjeagenda_EXPORT');

            $print_link = 'index.php?option=com_planjeagenda&view=attendees&task=attendees.export&tmpl=raw&id=' . $eventid . '&' . Session::getFormToken() . '=1';
            $output = '<a href="' . Route::_($print_link) . '" ' . self::tooltip($text, $overlib, 'editlinktip', 'bottom') . '>' . $image . '</a>';
        }
    ...
    static public function backbutton($backlink, $view)
    {
        ...
        $image = PlanjeagendaHtml::icon( 'com_planjeagenda/icon-16-back.webp', 'fa fa-fw fa-lg fa-chevron-circle-left klevents-backbutton', Text::_('com_planjeagenda_BACK'), NULL, !$app->isClient('site'));
        ...
        if ($app->input->get('print','','int')) {
            ...
            //button in view
            $overlib = Text::_('com_planjeagenda_BACK');
            $text = Text::_('com_planjeagenda_BACK');

            $link = 'index.php?option=com_planjeagenda&view='.$view.'&id='.$id.'&Itemid='.$fid.'&task='.$view.'.back';
    ...
    static public function mapicon($data, $view, $params)
    {
        ...
        //Link to map
        $mapimage = PlanjeagendaHtml::icon( 'com_planjeagenda/map_icon.webp', 'fa fa-map', Text::_('com_planjeagenda_MAP'), 'class="klevents-mapicon"');
        ...
        switch ($mapserv)
        {
            case 1:
                ...
                $message = Text::_('com_planjeagenda_MAP').':';
                $attributes = ' rel="{handler: \'iframe\', size: {x: 800, y: 500}}" latitude="" longitude=""';
                $output = '<dt class="venue_mapicon">'.$message.'</dt><dd class="venue_mapicon"><a class="flyermodal mapicon" title="'.Text::_('com_planjeagenda_MAP').'" target="_blank" href="'.$url.'"'.$attributes.'>'.$mapimage.'&nbsp;'.Text::sprintf('com_planjeagenda_LINK_TO_GOOGLE_MAP', $data->venue) .'</a></dd>';
                break;
            ...
            case 3:
                ...
                if (empty($api) || empty($clientid)) {
                    $output = Text::_('com_planjeagenda_GOOGLE_NO_API_KEY');
                    break;
                } else {
                    $api = trim($api);
                ...
                PlanjeagendaHelper::loadCss('googlemap');

                 $wa = $app->getDocument()->getWebAssetManager();
                 $wa->registerScript('klevents.infobox', 'com_planjeagenda/infobox.js')->useScript('klevents.infobox');
                 $wa->registerScript('klevents.googlemap', 'com_planjeagenda/googlemap.js')->useScript('klevents.googlemap');
            ...
            case 4:
                ...
                $message = Text::_('com_planjeagenda_MAP') . ':';
                $output = '<dt class="venue_mapicon">' . $message . '</dt><dd class="venue_mapicon"><a class="flyermodal mapicon" title="' . Text::_('com_planjeagenda_MAP') . '" target="_blank" href="' . $url . '">' . $mapimage . '&nbsp;' . Text::sprintf('com_planjeagenda_LINK_TO_OSM', $data->venue) . '</a></dd>';
            ...
            case 5:
                ...
                $address = 'street=' . urlencode($data->street) . '&city=' . urlencode($data->city) . '&country=' . urlencode($data->country) . '&postalcode=' . urlencode($data->postalCode);
                 ...
                 $wa = $app->getDocument()->getWebAssetManager();
                 $wa->registerScript('klevents.osmreload', 'com_planjeagenda/osmreload.js')->useScript('klevents.osmreload');
                 ...
                 if ($lat && $lng) {
                    ...
                    $output = '<iframe width="500" height="250" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="https://www.openstreetmap.org/export/embed.html?bbox=' . htmlentities(($lng - 0.001)) . ',' . htmlentities(($lat - 0.001)) . ',' . htmlentities(($lng + 0.001)) . ',' . htmlentities(($lat + 0.001)) . '&amp;layer=mapnik&amp;zoom=' . $zoom . '&amp;marker=' . htmlentities($lat) . ',' . htmlentities($lng) . '"></iframe>';
                     ...
                     $output = '<p>' . Text::sprintf('com_planjeagenda_OSM_NO_MAP', $fallback_url) . '</p>';
                 }
            break;
        }
        return $output;
    ...
    public static function recurrenceicon($event, $showinline = true, $showtitle = true)
    {
        ...
        if (version_compare(JVERSION, '5.0.0', '>=')) {
            // Joomla 5 with Font Awesome 6
            $iconRecurrenceFirst = 'fa fa-fw fa-refresh klevents-recurrencefirsticon';
            $iconRecurrence      = 'fa fa-fw fa-refresh klevents-recurrenceicon';
        } else {
            // Joomla 4 with Font Awesome 5
            $iconRecurrenceFirst = 'fa fa-fw fa-sync klevents-recurrencefirsticon';
            $iconRecurrence      = 'fa fa-fw fa-sync klevents-recurrenceicon';
        }

        $first = !empty($item->recurrence_type) && empty($item->recurrence_first_id);

        $image = $first
            ? 'com_planjeagenda/icon-32-recurrence-first.svg'
            : 'com_planjeagenda/icon-32-recurrence.svg';
        ...
        if ($showtitle) {
            $attribs['title'] = Text::_(
                $first
                    ? 'com_planjeagenda_RECURRING_FIRST_EVENT_DESC'
                    : 'com_planjeagenda_RECURRING_EVENT_DESC'
            );
        }
        ...
        return self::makeIcon(
            $image,
            $icon,
            Text::_('com_planjeagenda_RECURRING_EVENT'),
            $attribs,
            !$app->isClient('site')
        );
    }
    ...
    {
        //$settings = PlanjeagendaHelper::globalattribs();  /// @todo use global setting to influence visibility of publish state icon?
        $app = Factory::getApplication();
        ...
        switch ($published) {
        case -2: // trashed
            $image = 'com_planjeagenda/trash.webp';
            $icon = 'fa fa-fw fa-lg fa-trash klevents-publishstateicon-trashed';
            $alt   = Text::_('JTRASHED');
            break;
        case  0: // unpublished F10C: fa-circle-o F070: fa-eye-slash, F192: fa-dot-circle-o
            $image = 'com_planjeagenda/publish_x.webp';
            $icon = 'fa fa-fw fa-lg fa-eye-slash klevents-publishstateicon-unpublished';
            $alt   = Text::_('JUNPUBLISHED');
            break;
        case  1: // published F06E: fa-eye
            $image = 'com_planjeagenda/publish.webp';
            $icon = 'fa fa-fw fa-lg fa-check-circle klevents-publishstateicon-published';
            $alt   = Text::_('JPUBLISHED');
            break;
        case  2: // archived
            $image = 'com_planjeagenda/archive_front.webp';
            $icon = 'fa fa-fw fa-lg fa-archive klevents-publishstateicon-archived';
        ...
        $output = PlanjeagendaHtml::icon( $image, $icon, $alt, $attributes, !$app->isClient('site'));
    ...
    static public function flyer($data, $image, $type, $id = null)
    {
        ...
        switch($type) {
            case 'event':
                ...
                $info = $data->title;
                if(!$settings->flyer){
                    $precaption = Text::_('com_planjeagenda_EVENT');
                    $id = 'eventid-'. $data->id;
                }
                break;
            ...
            case 'category':
                ...
                $info = $data->catname;
                if(!$settings->flyer){
                    $precaption = Text::_('com_planjeagenda_CATEGORY');
                    $id = 'catid-'. $data->id;
                }
                break;
            ...
            case 'venue':
                ...
                $info = $data->venue;
                if(!$settings->flyer){
                    $precaption = Text::_('com_planjeagenda_VENUE');
        ...
        if (!$settings->flyer){
            if (is_file(JPATH_SITE.'/images/klevents/'.$folder.'/small/'.$imagefile)) {
                ...
                elseif (($settings->gddisabled == 1) && ($settings->lightbox == 0)) {
                    ...
                    $icon = '<img '.$attributes.' src="'.$uri->base().$image['thumb'].'" width="'.$thumbwidth.'" height="'.$thumbheight.'" alt="'.$info.'" title="'.Text::_('com_planjeagenda_CLICK_TO_ENLARGE').'" />';
                    $output = '<div class="flyerimage">'.$icon.'</div>';
                }
                ...
                elseif (($settings->gddisabled == 1) && ($settings->lightbox == 1)) {
                    ...
                    $icon = '<img class="example-thumbnail" itemprop="image" src="'.$uri->base().$image['thumb'].'" alt="'.$info.'" title="'.Text::_('com_planjeagenda_CLICK_TO_ENLARGE').'" />';
    ...
    {
        $settings = PlanjeagendaHelper::config();
        $check    = PlanjeagendaHelper::isValidDate($date);
        //$timezone = PlanjeagendaHelper::getTimeZoneName();
        $timezone = null;
    ...
    {
        $settings = PlanjeagendaHelper::config();
        $check    = PlanjeagendaHelper::isValidTime($time);
    ...
    static public function formatDateTime($dateStart, $timeStart ='', $dateEnd = '', $timeEnd = '', $dateFormat = '', $timeFormat = '', $addSuffix = true, $showTime = true, $showDayLink = false)
    {
        ...
        if (PlanjeagendaHelper::isValidDate($dateStart)) {
            $output .= '<span class="jem_date-1">';
            if ($showDayLink) {
                $output .= '<a href="'.Route::_(PlanjeagendaHelperRoute::getRoute(str_replace('-', '', $dateStart), 'day')).'">';
            }
            ...
            if ($showTime && PlanjeagendaHelper::isValidTime($timeStart)) {
            ...
            $displayDateEnd = PlanjeagendaHelper::isValidDate($dateEnd) && $dateEnd != $dateStart;
            if ($displayDateEnd) {
                $output .= '<span class="jem_date2"> - ';
                if ($showDayLink) {
                    $output .= '<a href="'.Route::_(PlanjeagendaHelperRoute::getRoute(str_replace('-', '', $dateEnd), 'day')).'">';
                }
            ...
            if ($showTime && PlanjeagendaHelper::isValidTime($timeStart) && PlanjeagendaHelper::isValidTime($timeEnd))
            {
                $output .= $displayDateEnd ? ', ' : ' - ';
                $output .= '<span class="jem_time-2">'.self::formattime($timeEnd, $timeFormat, $addSuffix).'</span>';
            }
        } else {
            $output .= '<span class="jem_date-1">'.Text::_('com_planjeagenda_OPEN_DATE').'</span>';

            if ($showTime) {
                if (PlanjeagendaHelper::isValidTime($timeStart)) {
                    $output .= '<span class="jem_time-1">, '.self::formattime($timeStart, $timeFormat, $addSuffix).'</span>';

                    // Display end time only when both times are set
                    if (PlanjeagendaHelper::isValidTime($timeEnd)) {
    ...
    static public function formatDateTime2Lines($dateStart, $timeStart = '', $dateEnd = '', $timeEnd = '', $dateFormat = '', $timeFormat = '', $addSuffix = true, $showTime = true)
    {
        ...
        $output = '';
        $jemconfig = PlanjeagendaHelper::config();

        if (empty($dateFormat)) {
            // Use format saved in settings if specified or format in language file otherwise
            $dateFormat = empty($jemconfig->formatShortDate) ? Text::_('com_planjeagenda_FORMAT_SHORT_DATE') : $jemconfig->formatShortDate;
        }

        if (PlanjeagendaHelper::isValidDate($dateStart)) {
            $outDate = self::formatdate($dateStart, $dateFormat);

            if (PlanjeagendaHelper::isValidDate($dateEnd) && ($dateEnd != $dateStart)) {
                $outDate .= ' - ' . self::formatdate($dateEnd, $dateFormat);
            }
        } else {
            $outDate = Text::_('com_planjeagenda_OPEN_DATE');
        }
    ...
    static public function formatShortDateTime($dateStart, $timeStart = '', $dateEnd = '', $timeEnd = '', $showTime = true)
    {
        ...
        // Use format saved in settings if specified or format in language file otherwise
        $params['dateFormat'] = (isset($settings->formatShortDate) && $settings->formatShortDate) ? $settings->formatShortDate : Text::_('com_planjeagenda_EVENTS_FORMAT_SHORT_DATE');
    ...
    static public function formatSchemaOrgDateTime($dateStart, $timeStart = '', $dateEnd = '', $timeEnd = '', $showTime = true)
    {
        ...
        if (PlanjeagendaHelper::isValidDate($dateStart)) {
            ...
            if (PlanjeagendaHelper::isValidDate($dateEnd)) {
    ...
    static public function getCategoryList($categories, $doLink, $backend = false)
    {
        $output = array_map(
            function ($category) use ($doLink, $backend) {
                if ($doLink) {
                    if ($backend) {
                        $path = $category->path ?? $category->catname ?? '';
                        $path = str_replace('/', ' &#187; ', $path);
                        $value  = '<span ' . self::tooltip(Text::_('com_planjeagenda_EDIT_CATEGORY'), $path, 'editlinktip') . '>';
                        $value .= '<a href="index.php?option=com_planjeagenda&amp;task=category.edit&amp;id=' . $category->id . '">' .
                                      $category->catname . '</a>';
                        $value .= '</span>';
                    } else {
...
/* @todo FIX
                        $catUrl = RouteHelper::getCategoryRoute(
                            $row->id . ':' . $row->alias,
                            $row->catid ?? 0,
                            $row->language ?? null
                            );
                        */
                        ...
                        $value  = '<a href="' . Route::_(RouteHelper::getCategoryRoute($category->catslug)) . '">' .
                                      $category->catname . '</a>';
                    }
                } else {
    ...
    
    {
        ...
        $states = array(
            -99 => array( // fallback on wrong status value
                ...
                'fa-circle',
                '',
                'com_planjeagenda_STATUS_UNKNOWN',
                'com_planjeagenda_STATUS_UNKNOWN',
                'com_planjeagenda_ATTENDEES_STATUS_UNKNOWN'
            ),
            -1 => array( // not attending, no toggle
                'publish_r.webp',
                'fa-times-circle',
                '',
                'com_planjeagenda_NOT_ATTENDING',
                'com_planjeagenda_NOT_ATTENDING',
                'com_planjeagenda_ATTENDEES_NOT_ATTENDING'
            ),
            0 => array( // invited, no toggle
                'invited.webp',
                'fa-question-circle',
                '',
                'com_planjeagenda_INVITED',
                'com_planjeagenda_INVITED',
                'com_planjeagenda_ATTENDEES_INVITED'
            ),
            1 => array( // attending, toggle: waiting list
                'tick.webp',
                'fa-check-circle',
                'attendees.OnWaitinglist',
                'com_planjeagenda_ATTENDING',
                'com_planjeagenda_ATTENDING_MOVE_TO_WAITINGLIST',
                'com_planjeagenda_ATTENDEES_ATTENDING'
            ),
            2 => array( // on waiting list, toggle: list of attendees
                'publish_y.webp',
                'fa-hourglass-half', //fa-exclamation-circle',
                'attendees.OffWaitinglist',
                'com_planjeagenda_ON_WAITINGLIST',
                'com_planjeagenda_ON_WAITINGLIST_MOVE_TO_ATTENDING',
                'com_planjeagenda_ATTENDEES_ON_WAITINGLIST'
            )
        );
        ...
        HTMLHelper::_('bootstrap.tooltip');
        $attr = 'class="hasTooltip" data-bs-toggle="tooltip" title="'.HTMLHelper::tooltipText(Text::_('com_planjeagenda_STATUS'), $canChange ? Text::_($state[4]) : str_replace(" ", "&nbsp",Text::_($state[3])), 0).'"';
        
        if ($print) {
            $html = self::makeIcon('com_planjeagenda/'.$state[0], 'fa fa-fw fa-lg '.$state[1].' klevents-attendance-status-'.$state[1], $state[3], 'class="icon-inline-left"', $backend);
            $html .= Text::_($state[5]);
        } elseif ($canChange && !empty($state[2])) {
            $html = self::makeIcon('com_planjeagenda/'.$state[0], 'fa fa-fw fa-lg '.$state[1].' klevents-attendance-status-'.$state[1], $state[3], null, $backend);
            if ($backend) {
                $attr .= ' onclick="return Joomla.listItemTask(\'cb' . $i . '\',\'' . $state[2] . '\')"';
                $url = '#';
            } else {
                $url = Route::_('index.php?option=com_planjeagenda&view=attendees&amp;task=attendees.attendeetoggle&id='.$i.'&'.Session::getFormToken().'=1');
            }
            $html = HTMLHelper::_('link', $url, $html, $attr);
        } else {
            $html = self::makeIcon('com_planjeagenda/'.$state[0], 'fa fa-fw fa-lg '.$state[1].' klevents-attendance-status-'.$state[1], $state[3], $attr, $backend);
        }
