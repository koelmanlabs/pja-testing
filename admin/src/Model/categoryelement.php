...
class CategoryelementModel extends BaseDatabaseModel
{
    /**
     * Pagination object
     *
    ...
    public function getData()
    {
        ...
        if ($search) {
            $query = 'SELECT c.id FROM #__pja_categories AS c' . ' WHERE LOWER(c.catname) LIKE ' . $db->Quote('%' . $this->_db->escape($search, true) . '%', false) . $where2;
            $db->setQuery($query);
            $search_rows = $db->loadColumn();
        }

        $query = 'SELECT c.*, u.name AS editor, g.title AS groupname, gr.name AS catgroup'
                . ' FROM #__pja_categories AS c' . ' LEFT JOIN #__viewlevels AS g ON g.id = c.access'
                . ' LEFT JOIN #__users AS u ON u.id = c.checked_out'
                . ' LEFT JOIN #__pja_groups AS gr ON gr.id = c.groupid'
                . $where
        ...
        $list = PlanjeagendaCategories::treerecurse($parentid, '', array(), $children, 9999, 0, 0);
