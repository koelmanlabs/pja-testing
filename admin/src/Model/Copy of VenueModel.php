 ...
 * Model: Venue
 */
class PlanjeagendaModelVenue extends PlanjeagendaModelAdmin
{
    /**
     * Constructor
     */
    ...
    public function delete(&$pks = array())
    {
        ...
        if ($pks)
        {
            ...
            foreach ($pks as $pk)
            {
                ...
                $query->from($db->quoteName('#__pja_venues').' AS v');
                $query->join('LEFT', '#__pja_events AS e ON e.locid = v.id');
                ...
                if ($assignedEvents > 0)
                {
                    $result[] = Text::_('com_planjeagenda_VENUE_ASSIGNED_EVENT');
                }
     ...
     */
    public function getTable($type = 'Venue', $prefix = 'PlanjeagendaTable', $config = array())
    {
    ...
    {
        // Ensure the form path is registered
    ...
    public function getItem($pk = null)
    {
        ...
        if ($item = parent::getItem($pk)) {
            $files = PlanjeagendaAttachment::getAttachments('venue'.$item->id);
    ...
    protected function loadFormData()
    {
        ...
        $data = Factory::getApplication()->getUserState('com_planjeagenda.edit.venue.data', array());
    ...
    {
        PlanjeagendaDebug::log('venue save', 'id=' . ($data['id'] ?? 'new') . ' name=' . ($data['venue'] ?? ''));
        // Variables
        ...
        if ($saved) {
            ...
            if ($allowed) {
                ...
                AttachmentHelper::postUpload($attachments, 'venue', $pk);
            }
            ...
            foreach ($old['id'] as $k => $id){
                ...
                PlanjeagendaAttachment::update($attach);
            }
        }
