...
class AttendeesModel extends ListModel
{
    ...
    protected function populateState($ordering = null, $direction = null)
    {
        ...
        $limit      = $app->getUserStateFromRequest('com_planjeagenda.attendees.limit', 'limit', $app->get('list_limit'), 'int');
        $limitstart = $app->getUserStateFromRequest('com_planjeagenda.attendees.limitstart', 'limitstart', 0, 'int');
    ...
    protected function getListQuery()
    {
        ...
        $query->from($db->quoteName('#__pja_register').' AS r');

        // Join event data
        $query->select('a.waitinglist AS waitinglist');
        $query->join('LEFT', '#__pja_events   AS a ON (r.event = a.id)');
    ...
    // TODO: filter status
        ...
        $allowed_attendee_cols = ['r.id','u.name','u.username','u.email','r.status','r.places','r.uregdate'];
        $orderCol = PlanjeagendaHelper::sanitizeOrderCol($orderCol, $allowed_attendee_cols, 'u.name');
        $_allowed = ['u.name','u.username','u.email','r.status','r.places','r.uregdate','r.uip'];
        $orderCol = PlanjeagendaHelper::sanitizeOrderCol($orderCol, $_allowed, 'u.name');
    ...
    public function remove($cid = array())
    {
        if (is_array($cid) && count($cid))
        {
            ...
            $query = $db->getQuery(true);
            $query->delete($db->quoteName('#__pja_register'));
    ...
    public function getCsv()
    {
        ...
        $header = array(
                Text::_('com_planjeagenda_NAME'),
                Text::_('com_planjeagenda_USERNAME'),
                Text::_('com_planjeagenda_EMAIL'),
                Text::_('com_planjeagenda_REGDATE'),
                Text::_('com_planjeagenda_ATTENDEES_PLACES'),
                Text::_('com_planjeagenda_HEADER_WAITINGLIST_STATUS')
            );
        if ($comments) {
            $header[] = Text::_('com_planjeagenda_COMMENT');
        }
        $header[] = Text::_('com_planjeagenda_ATTENDEES_REGID');
        ...
        foreach ($items as $item)
        {
            ...
            if ($status < 0) {
                ...
                $txt_stat = $item->waiting ? 'com_planjeagenda_ATTENDEES_ON_WAITINGLIST' : 'com_planjeagenda_ATTENDEES_ATTENDING';
            } else {
