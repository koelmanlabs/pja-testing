...
// ensure PlanjeagendaFactory is loaded (because field maybe used by modules too)
...
class JFormFieldCategoryEdit extends ListField
{
    ...
    protected function getOptions()
    {
        ...
        $query->join('LEFT', $db->quoteName('#__pja_categories').' AS b ON a.lft > b.lft AND a.rgt < b.rgt');
        ...
        // To rearrange parents and children move the children up, not the parents down.
            $query->join('LEFT', $db->quoteName('#__pja_categories').' AS p ON p.id = '.(int) $oldCat);
            $query->where('NOT(a.lft >= p.lft AND a.rgt <= p.rgt)');

            $rowQuery    = $db->getQuery(true);
            $rowQuery->select('a.id AS value, a.catname AS text, a.level, a.parent_id');
            $rowQuery->from('#__pja_categories AS a');
        ...
        if ($oldCat == 0)
        {
            foreach ($options as $i => $option)
            {
                ...
                if ($user->authorise('core.create', 'com_planjeagenda' . '.category.' . $option->value) != true )
                {
                    unset($options[$i]);
                }
            }
        ...
        else
        {
            ...
            foreach ($options as $i => $option)
            {
                if ($user->authorise('core.edit.state', 'com_planjeagenda' . '.category.' . $oldCat) != true && !isset($oldParent))
                {
                ...
                if ($user->authorise('core.edit.state', 'com_planjeagenda' . '.category.' . $oldCat) != true
