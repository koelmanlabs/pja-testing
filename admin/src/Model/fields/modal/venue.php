...
class JFormFieldModal_Venue extends FormField
{
    ...
    protected function getInput()
    {
        ...
        $html = array();
        $link = 'index.php?option=com_planjeagenda&amp;view=venueelement&amp;tmpl=component&amp;function=jSelectVenue_'.$this->id;

        $db = Factory::getContainer()->get('DatabaseDriver');
        $query = $db->getQuery(true);
        $query->select('venue');
        $query->from('#__pja_venues');
        ...
        if (empty($venue)) {
            $venue = Text::_('com_planjeagenda_SELECTVENUE');
        }
        ...
        // $html[] = '    <a class="modal" title="'.Text::_('com_planjeagenda_SELECT').'" href="'.$link.'&amp;'.Session::getFormToken().'=1" rel="{handler: \'iframe\', size: {x:800, y:450}}">'.
        //             Text::_('com_planjeagenda_SELECT').'</a>';
        $html[] = HTMLHelper::_(
        ...
        $html[] ='<button type="button" class="btn btn-link btn-primary" data-bs-dismiss="modal" data-bs-toggle="modal" data-bs-target="#venue-modal-1">'.Text::_('com_planjeagenda_SELECT').'
        </button>';
        $html[] = '  </div>';
        $html[] = '</div>';
