...
class CategoryModel extends AdminModel
{
    ...
    protected function canEditState($record)
    {
        ...
        if (!empty($record->id)) {
            return $user->authorise('core.edit.state', 'com_planjeagenda' . '.category.' . (int) $record->id);
        }
        // New category, so check against the parent.
        elseif (!empty($record->parent_id)) {
            return $user->authorise('core.edit.state', 'com_planjeagenda' . '.category.' . (int) $record->parent_id);
        }
     ...
     */
    public function getTable($type = 'Category', $prefix = 'Table', $config = [])
    {
        try {
            $db = Factory::getContainer()->get('DatabaseDriver');
            return new \KoelmanLabs\Component\Planjeagenda\Administrator\Table\CategoryTable($db);
        } catch (\Exception $e) {
    ...
    public function getItem($pk = null)
    {
        if ($result = parent::getItem($pk))
        {
            ...
            $tz = new \DateTimeZone(\Joomla\CMS\Factory::getConfig()->get('offset', 'UTC'));
    ...
    public function getForm($data = array(), $loadData = true)
    {
        ...
        $form = $this->loadForm('com_planjeagenda.category', 'category',
    ...
    protected function loadFormData()
    {
        ...
        $data = Factory::getApplication()->getUserState('com_planjeagenda.edit.category.data', array());
    ...
    public function save($data)
    {
        ...
        // $result = Factory::getApplication()->triggerEvent($this->event_before_save, array($this->option . '.' . $this->name, &$table, $isNew));
        $result = Factory::getApplication()->triggerEvent($this->event_before_save, array($this->option . '.' . $this->name, &$table, $isNew,''));
     ...
     *
     * TODO: Should this clean caches of JEM, e.g. com_planjeagenda and mod_jem* ?
     */
    ...
    public function delete(&$cids)
    {
        ...
        if (count($cid) && count($err) == 0) {
            $cids = implode(',', $cid);
            $query = 'DELETE FROM #__pja_categories'
                   . ' WHERE id IN (' . $cids . ')';
        ...
        if (count($err)) {
            $cids = implode(', ', $err);
            $msg = Text::sprintf('com_planjeagenda_EVENT_ASSIGNED_CATEGORY', $cids);
            return $msg;
        }
        else {
            $total = count($cid);
            $msg = Text::plural('com_planjeagenda_CATEGORIES_N_ITEMS_DELETED', $total);
            return $msg;
        }
    }
