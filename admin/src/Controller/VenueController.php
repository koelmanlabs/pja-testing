...
class VenueController extends FormController
{
    ...
    public function upload()
    {
        ...
        $app   = Factory::getApplication();
        $input = $app->input;
        $file  = $input->files->get('file'); // Dropzone stuurt standaard 'file'
        $id    = $input->getInt('id');       // Venue ID
        
        $res = ['success' => false];
        
        if (!empty($file['name'])) {
            // Gebruik je bestaande AttachmentHelper om het bestand fysiek op te slaan
            // We sturen de 'file' array direct door naar de upload helper
            $uploaded = AttachmentHelper::postUpload($file, 'venue', $id);
    ...
    public function cancel($key = null)
    {
        ...
        if ($id) {
            try {
                ...
                if ($model && method_exists($model, 'checkin')) { $model->checkin($id); }
            } catch (\Throwable $e) {}
        }
        $this->setRedirect(\Joomla\CMS\Router\Route::_('index.php?option=com_planjeagenda&view=venues', false));
        return true;
    }
