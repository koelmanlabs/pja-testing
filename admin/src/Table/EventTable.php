...
class EventTable extends Table
{
    ...
    {
        return 'com_planjeagenda.event.' . (int) $this->id;
    }
    ...
    public function bind($array, $ignore = '')
    {
        ...
        // Set catid from the cats array (first category) or default to 0
        if (!isset($array['catid']) || empty($array['catid'])) {
            if (!empty($array['cats'])) {
                $cats = is_array($array['cats']) ? $array['cats'] : explode(',', $array['cats']);
                $array['catid'] = (int) reset($cats);
            } else {
                $array['catid'] = 0;
            }
        }
    ...
    public function check()
    {
        ...
        if (trim($this->title) == '') {
            $this->setError(Text::_('com_planjeagenda_EVENT_ERROR_NAME'));
            return false;
        }
        ...
        if (empty($this->alias)) {
            $this->alias = PlanjeagendaHelper::stringURLSafe($this->title);
        ...
        // Check if end date is before start date — wrapped in try/catch for invalid date strings.
        try {
            ...
            if ($date1 > $date2) {
                $this->setError(Text::_('com_planjeagenda_EVENT_ERROR_END_BEFORE_START_DATES'));
                return false;
            }
        } catch (\Exception $e) {
            // Invalid date/time string — skip comparison, log for admin review.
            \Joomla\CMS\Log\Log::add('JEM event date check failed: ' . $e->getMessage(), \Joomla\CMS\Log\Log::WARNING, 'JEM');
        }
    ...
    public function store($updateNulls = true)
    {
        ...
        $image_dir = JPATH_SITE.'/images/klevents/events/';
        ...
        if ($ret && $image_to_delete) {
            \PlanjeagendaHelper::delete_unused_image_files('event', $image_to_delete);
        }
    ...
    public function delete($pk = null)
    {
        ...
        if (parent::delete($pk)) {
            ...
            $query->delete($db->quoteName('#__pja_cats_event_relations'));
