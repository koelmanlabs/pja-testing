...
class jem_events extends Table
{
    ...
    */
    ...
    public function check($jemsettings = null)
    {
        ...
        if ($this->title == '') {
            $this->_error = Text::_('com_planjeagenda_ADD_TITLE');
            Factory::getApplication()->enqueueMessage($this->_error, 'warning');
            return false;
        }

        if ($titlelength > 100) {
            $this->_error = Text::_('com_planjeagenda_ERROR_TITLE_LONG');
            Factory::getApplication()->enqueueMessage($this->_error, 'warning');
            return false;
        }

        $alias = \Joomla\CMS\Filter\OutputFilter::stringURLSafe($this->title);
        ...
        if ($this->dates && !preg_match("/^[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]$/", $this->dates)) {
            $this->_error = Text::_('com_planjeagenda_DATE_WRONG');
            Factory::getApplication()->enqueueMessage($this->_error, 'warning');
            return false;
        }
        ...
        if (isset($this->enddates)) {
            if (!preg_match("/^[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]$/", $this->enddates)) {
                $this->_error = Text::_('com_planjeagenda_ENDDATE_WRONG_FORMAT');
                Factory::getApplication()->enqueueMessage($this->_error, 'warning');
                return false;
            }
        }
...
/*        if (isset($this->recurrence_limit_date)) {
            if (!preg_match("/^[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]$/", $this->recurrence_limit_date)) {
                     $this->_error = Text::_('com_planjeagenda_WRONGRECURRENCEDATEFORMAT');
                     Factory::getApplication()->enqueueMessage($this->_error, 'warning');
                     return false;
            }
        }
        */
        ...
        if (isset($this->endtimes) && $this->endtimes) {
            if (!preg_match("/^[0-2][0-9]:[0-5][0-9](:[0-5][0-9])?$/", $this->endtimes)) {
                $this->_error = Text::_('com_planjeagenda_WRONGENDTIMEFORMAT');
                Factory::getApplication()->enqueueMessage($this->_error, 'warning');
                return false;
            }
        }
        ...
        //if ($this->locid == '') {
        //    $this->_error = Text::_('com_planjeagenda_VENUE_EMPTY');
        //    Factory::getApplication()->enqueueMessage($this->_error, 'warning');
        //    return false;
        //}
