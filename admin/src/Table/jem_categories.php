...
class jem_categories extends Nested
{
    ...
    public function check()
    {
        ...
        if (trim($this->catname) == '') {
            $this->_error = Text::_('com_planjeagenda_ADD_NAME_CATEGORY');
            Factory::getApplication()->enqueueMessage($this->_error, 'warning');
            return false;
        }

        $alias = \Joomla\CMS\Filter\OutputFilter::stringURLSafe($this->catname);
        ...
        $query = 'SELECT id FROM #__pja_categories WHERE catname = '.$this->_db->Quote($this->catname);
        $this->_db->setQuery($query);

        $xid = intval($this->_db->loadResult());
        if ($xid && $xid != intval($this->id)) {
            Factory::getApplication()->enqueueMessage(Text::sprintf('com_planjeagenda_CATEGORY_NAME_ALREADY_EXIST', $this->catname), 'warning');
            return false;
        }
        */
