...
class jem_groups extends Table
{
    ...
    {
        parent::__construct('#__pja_groups', 'id', $db);
    }
    ...
    public function check()
    {
        ...
        if (trim($this->name) == '') {
            $this->_error = Text::_('com_planjeagenda_ADD_GROUP_NAME');
            Factory::getApplication()->enqueueMessage($this->_error, 'warning');
            return false;
        }

        // check for existing name
        $query = 'SELECT id FROM #__pja_groups WHERE name = '.$this->_db->Quote($this->name);
        $this->_db->setQuery($query);

        $xid = intval($this->_db->loadResult());
        if ($xid && $xid != intval($this->id)) {
            Factory::getApplication()->enqueueMessage(Text::sprintf('com_planjeagenda_GROUP_NAME_ALREADY_EXIST', $this->name), 'warning');
            return false;
        }
