...
class jem_venues extends Table
{
    ...
    public function check()
    {
        ...
        if (!trim($this->venue)) {
            $this->_error = Text::_('com_planjeagenda_ADD_VENUE');
            Factory::getApplication()->enqueueMessage($this->_error, 'warning');
            return false;
        }

        $alias = \Joomla\CMS\Filter\OutputFilter::stringURLSafe($this->venue);
        ...
        if ($this->map) {
            if (!trim($this->street) || !trim($this->city) || !trim($this->country) || !trim($this->postalCode)) {
                if ((!trim($this->latitude) && !trim($this->longitude))) {
                    $this->_error = Text::_('com_planjeagenda_ERROR_ADDRESS');
                    Factory::getApplication()->enqueueMessage($this->_error, 'warning');
                    return false;
                }
            }
        ...
        if (\Joomla\CMS\Filter\InputFilter::checkAttribute(array ('href', $this->url))) {
            $this->_error = Text::_('com_planjeagenda_ERROR_URL_WRONG_FORMAT');
            Factory::getApplication()->enqueueMessage($this->_error, 'warning');
            return false;
        }
        ...
        if (\Joomla\String\StringHelper::strlen($this->street) > 50) {
            $this->_error = Text::_('com_planjeagenda_ERROR_STREET_LONG');
            Factory::getApplication()->enqueueMessage($this->_error, 'warning');
            return false;
        }
        ...
        if (\Joomla\String\StringHelper::strlen($this->postalCode) > 10) {
            $this->_error = Text::_('com_planjeagenda_ERROR_ZIP_LONG');
            Factory::getApplication()->enqueueMessage($this->_error, 'warning');
            return false;
        }
        ...
        if (\Joomla\String\StringHelper::strlen($this->city) > 50) {
            $this->_error = Text::_('com_planjeagenda_ERROR_CITY_LONG');
            Factory::getApplication()->enqueueMessage($this->_error, 'warning');
            return false;
        }
        ...
        if (\Joomla\String\StringHelper::strlen($this->state) > 50) {
            $this->_error = Text::_('com_planjeagenda_ERROR_STATE_LONG');
            Factory::getApplication()->enqueueMessage($this->_error, 'warning');
            return false;
        }
        ...
        if (\Joomla\String\StringHelper::strlen($this->country) > 2) {
            $this->_error = Text::_('com_planjeagenda_ERROR_COUNTRY_LONG');
            Factory::getApplication()->enqueueMessage($this->_error, 'warning');
            return false;
        }
        ...
        /*
        $query = 'SELECT id FROM #__pja_venues WHERE venue = '.$this->_db->Quote($this->venue);
        $this->_db->setQuery($query);

        $xid = intval($this->_db->loadResult());
        if ($xid && $xid != intval($this->id)) {
            Factory::getApplication()->enqueueMessage(Text::sprintf('com_planjeagenda_VENUE_NAME_ALREADY_EXIST', $this->venue), 'warning');
            return false;
        }
        */
